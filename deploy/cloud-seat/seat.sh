#!/bin/bash
# Cloud seat — the SEAT phase, run as the unprivileged `seat` user (ADR 390). entrypoint.sh brought
# the tailnet up as root, resolved the hub, and exec'd here with HOME, HUB_IP and LOG_DIR set. From
# this line on nothing on the machine runs as root except tailscaled.
#
# Every step is a no-op once done: daemon → team → enroll → seat → wake.
set -euo pipefail

: "${MUSTERD_TEAM:?}" "${MUSTERD_HUB:?}" "${MUSTERD_SEAT:?}" "${HUB_IP:?}" "${HOME:?}" "${LOG_DIR:?}"
...
# The invite was consumed at enrollment (first boot) and has no further use; the actuator's children
# (`claude -p`, the seat's own session) inherit this environment, so it leaves before they exist.
# GH_TOKEN and the model credential stay: the seat's work needs them.
unset MUSTERD_INVITE TAILSCALE_AUTHKEY
log "wake actuator starting for seat $MUSTERD_SEAT (uid $(id -u), not root)"
