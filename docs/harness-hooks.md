...
- **`UserPromptSubmit`** (fires when you send a prompt — a natural task boundary) — remind it to post a one-line `status_update` on the work it just finished (this is what flips the roster to `working:`) and to `team_inbox_check` for replies. Phrase it to self-filter ("_if you finished a unit of work_") so it nudges at real boundaries, not every turn. The managed install also runs `musterd session label-nudge` here — the due-gated half of the sidebar label sweep (ADR 160): one imperative line per turn while no sweep has stamped the machine-wide file in the last 4h, silence otherwise. Per-turn-until-done is deliberate: the one-shot SessionStart ask was measured to fail (agents skip it under a busy first prompt), and a repeating, self-quieting nudge is what actually gets the sweep run.
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "f=\"${CLAUDE_PROJECT_DIR:-.}/AGENTS.md\"; test -f \"$f\" && grep -q musterd:start \"$f\" || exit 0; echo 'musterd: if you finished a unit of work since your last update, post a one-line team_send status_update (flips you to working: on the roster); then team_inbox_check for replies.'; command -v musterd >/dev/null 2>&1 && musterd session label-nudge 2>/dev/null || true"
          }
        ]
      }
    ]
