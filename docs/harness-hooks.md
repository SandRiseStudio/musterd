...
**Auto-installed, project-local (`musterd init`/`musterd agent`).** As of musterd/0.3, `configure` installs a **project-local** version of this same `SessionStart` verify-then-orient hook into `.claude/settings.local.json` (marker `# musterd-sessionstart-hook`), alongside the `Notification` hook below — so every provisioned folder self-reports drift and orients on launch with no manual step, and `musterd uninstall` removes it exactly. The global recipe above is still worth pasting for one thing the project-local install can't cover: a **fresh clone/worktree that was never provisioned here** — that folder has the committed primer but no local settings (and so no local hook), so only a global self-gating hook fires there. Use the auto-install for the common case; add the global recipe if you want every un-provisioned checkout to be told to run `musterd init`.
