...
- **Denominator moved today — and the window is a stack.** ADR 253 (agents-only live pick) + nick armed `stakes_defaults: packages/web/** → low`. No-risk web-only lanes now mostly route no ask. Size increments against **post-arming** ask volume, not the 14-day mix. That same window also contains #768, #771, #773, **and the arming itself**. Arming removes web lanes from acceptance: it changes the population, not only its speed. A 10-minute-rate miss still indicts increment 1 (disproof). A credit read — "increment 1 moved the rate" — must name the whole stack (stanley, #785 accept; same shape as the ADR 258 Eval amendment).
