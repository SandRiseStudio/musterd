...
describe('story on Goal (goals-front-door design)', () => {
  ...
  it('GoalSchema carries story through the read projection', () => {
    const g = GoalSchema.parse({
      id: 'g1', title: 't', wave: null, depends_on: [], declared_by: 'nick',
      declared_at: 1, status: 'planned', epoch: 0, story: 'plain words',
    });
...
describe('no_goal warning kind', () => {
  it('parses', () => {
    const w = LaneWarningSchema.parse({
      kind: 'no_goal', subject: 'L1', with: 'g1', owner: null,
...
- [ ] **Step 3: Implement.** In `goals.ts`: export
  ...
  `story: GoalStorySchema.optional()` (doc comment: *"plain-language one-liner for the
  stranger — what this goal means, not its title"*) to the `goal` object in
  ...
  LaneWarning doc comment (goals-front-door design: *"`no_goal` — a contending lane on no
  goal while the team has unshipped goals; advisory, owner-null, never woken"*). In
...
- [ ] **Step 3: Implement.** `GoalAccumulator` gains `story?: string`; the fold at :135
  sets `story: g.story ?? prior?.story` — wait, no: *a re-declaration replaces the skeleton
  wholesale* (comment at :143) — so `story: g.story` (undefined clears, latest wins,
...
describe('no_goal warning (goals-front-door design)', () => {
  ...
  it('never warns: attached lane / no goals declared / only shipped goals / backlog lane', () => {
    ...
    // attached:
    const linked = openLane(db, team.id, 'bravo', 'Cleo', { title: 'b', goal_id: 'g1', claim: true });
    expect(laneWarnings(db, team.id, 'bravo', linked).some((x) => x.kind === 'no_goal')).toBe(false);
  ...
  it('board projection carries it once per lane', () => {
    ...
    expect(boardWarnings(db, team.id, 'bravo', lanes).filter((w) => w.kind === 'no_goal')).toHaveLength(1);
  });
});
```
...
export function noGoalWarning(lane: Lane, goals: Goal[]): LaneWarning | null {
  ...
  return {
    kind: 'no_goal', subject: lane.id, with: suggest.id, owner: null,
...
**Rollout (post-merge, operational — not a code task):** on the revive team, declare the
active-arc goals with stories via `team_goal_declare` (native harness / wake pricing /
any-of asks / goals front door), then link in-flight lanes with `lane_update goal_id` —
or let the fresh `no_goal` warnings prompt owners to. The daemon auto-refreshes on merge;
never task nick with `service refresh`.
