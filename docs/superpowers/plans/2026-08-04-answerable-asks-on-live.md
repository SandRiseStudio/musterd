...
- **ADR number is 222**, allocated with `pnpm adr:next`. This arc has now lost **three** numbers: 219 to izzo (#628), 220 to stanley (#630), 221 to stanley again (#633). The first two were hand-reading `origin/main`, which #630 fixed. **The third happened WITH the tool**, because `pnpm adr:next` reads open PRs and this branch was never pushed — so a number claimed in an unpushed branch is invisible to everyone, including the tool. That is ADR 220's own pre-registered limit, and it means: **push the branch early, or expect to renumber.** Re-run the tool at PR time regardless. `pnpm adr-numbers:check` fails on a duplicate number or an H1 that does not match the filename.
...
| File                                            | Responsibility                                                                                                                |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `docs/decisions/222-answerable-asks-on-live.md` | Create — the decision, the measured zero baseline, the boundary.                                                              |
...
- Create: `docs/decisions/222-answerable-asks-on-live.md`
...
git add docs/decisions/222-answerable-asks-on-live.md
git commit -m "ADR 222: answerable asks on /live"
```
 ...
 * route** (ADR 222). /live and /board used to keep separate ideas of who you are, and /live's was
...
git commit -m "One member identity slot per browser per team (ADR 222)"
```
...
git commit -m "/board reads its identity from the shared slot (ADR 222)"
```
...
/**
 * The total precedence order for who this page connects as (ADR 222):
...
git commit -m "/live prefers a signed-in member over an observer (ADR 222)"
```
...
// construction (ADR 063). What is new (ADR 222) is that not-answerable is a *state we show*, not a
...
git commit -m "The asks rail says why it is read-only and offers the way in (ADR 222)"
```
...
git commit -m "Answer buttons follow the ask, not the sender (ADR 222)"
```
...
it("offers this machine's identity to a local page, and never off-machine (ADR 222)", async () => {
...
/**
 * `GET /teams/:slug/local-identity` (ADR 222) — hand a page on THIS machine the identity the CLI
...
git commit -m "The daemon offers this machine's sign-in identity, localhost only (ADR 222)"
```
...
```ts
/**
 * Ask the daemon whether THIS machine has a sign-in identity for the team (ADR 222). Never throws:
...
git commit -m "One-click sign-in on /live (ADR 222)"
```
...
it('signs into either surface with the same one-shot nonce (ADR 222)', () => {
...
git commit -m "musterd live — the CLI walks you into the office (ADR 222)"
