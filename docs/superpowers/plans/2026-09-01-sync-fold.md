...
- **v52 also indexes `messages(team_id, created_at)`** — the read-side move (a separate lane, hard precondition on the second machine) needs it and nothing indexes `created_at` today.
