...
- **`created_at` is local receipt time — `now`, never `envelope.ts`.** Spec §"The fold's rule" 4 and §"The ts-cursor defect": the readers that will move off `ts` move onto this column, so a wire value here reintroduces the defect. `from_provenance` comes from the wire; `origin_node`/`origin_seq` are copied verbatim.
- **The `messages(team_id, created_at)` index landed separately as v53 (ADR 349, PR #1161)** — it was drafted here as part of v52 — the read-side move (a separate lane, hard precondition on the second machine) needs it and nothing indexes `created_at` today.
...
| File | Responsibility |
| --- | --- |
| `packages/server/src/db/migrations.ts` | v54: `idx_messages_origin` unique index; `sync_pull_cursor` table |
...
### Task 1: Migration v54 — the fold's idempotence key and the pull cursor
...
```ts
  it('v54 holds (origin_node, origin_seq) unique in messages and adds the pull cursor', () => {
    ...
    expect(
      db.prepare("SELECT name FROM sqlite_master WHERE type = 'index' AND name = 'idx_messages_team_created'").get(),
    ).toBeDefined();
    db.close();
  });
```
...
```ts
  {
    ...
    up: (db) => {
      db.exec(`
        ...
        -- The read side's future key (spec §"The ts-cursor defect"): readers moving off ts land on
        -- created_at, and nothing in messages indexes it today.
...
git commit -m "feat(server): migration v54 — messages(origin_node, origin_seq) unique + sync_pull_cursor"
```
...
- Spec coverage: Finding 1 → Task 4; Finding 2 → Task 3 (block on from/to) + Task 6 (once-per-seat error); wire → Tasks 2, 5; fold rules 1–5 → Task 3; schema v54 → Task 1; errors table → Task 6 `reportStop` (all four kinds) + the 409 impossible-resume; testing section → Tasks 3, 6, 7; docs → Task 7.
