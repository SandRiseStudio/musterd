...
status:     void
resolution: "VOID — below the volume floor, exactly as pre-registered. ONE `guardian.sampled` row exists in the window 2026-09-05..2026-09-12 (2026-09-06 08:49:47, pid 55049) against a floor of 5; the log carries 378 lines and 4 sampled rows in total, the other three all on 2026-09-14, outside the window. Read 2026-09-14 by ryder (izzo opened it). NO VERDICT is read from this: below the floor the read measures how often the daemon stalled, not what the sample said, and the pre-commitment says it MUST NOT be read as the falsifier. What the week did measure is volume: #1308's prevention took the clean-exit-unreachable rate from 7–13 ticks a day (2026-09-01..04) to ONE in seven days, so the floor of 5 was unreachable in a week. The other two `void_if` clauses did not fire: the only guardian change in the window is e4f5a5e5 itself (2026-09-05 17:04, the parser correction the watch cites), the checkout bounced onto 0a636599 at 17:59 that day (contains e4f5a5e5), so the 09-06 row carries the NEW parser's verdict; CONFIRM_TIMEOUT_MS and the tick cadence are untouched. Recorded for the successor, NOT as a result: the one in-window row reads `wedged: true, frame: ???, entry: null, share: 1.0, samples: 1927, promoted: false` and is followed 181 s later by `guardian.stall_recovered` (unreachable 195,725 ms) with no restart — the falsifier's exact shape. So are all three 2026-09-14 rows (share 0.996/0.999/1.0; unreachable 233 s, 171 s, 157 s in total, each recovered with no restart). `???` is V8 JIT code (`<unknown binary>`, a RUNTIME_IMAGE), so the held chain never left the runtime: the main thread was running JavaScript, which ADR 389 §1 as restated on 2026-09-05 calls held, not parked — the parser applied its rule correctly and the daemon healed anyway. Four of four rows to date are this shape; whether that disarms the class is the successor's question, at the floor. Successor: 2026-09-14-adr-389-sampled-read-successor.md, thirty days, same floor, same falsifier."
---
