...
status:     void
resolution: "VOID — population unstable, by this watch's own first `void_if`: five workspaces joined the binding registry inside the window (agents-big-body, agents-ghost, agents-schmidt, agents-sloane, agents-compo, plus fix-relay-session-diagnostic), so the sampled set was never the nine named. Read anyway, as the count over the nine named workspaces (which were present in every one of the 3,890 samples, 2026-08-21T00:01Z to 2026-09-10T20:44Z): ONE demote, agents-dolly, 2026-08-26T05:38:51Z, single sample, self-corrected at the next sample. Per-case inspection: NOT the Cursor trust-gap shape. No Cursor session existed — `~/.cursor/projects/Users-nick-agents-dolly/.workspace-trusted` was born 2026-09-06T00:21Z, eleven days later, and the Cursor transcript dir was last touched 07-30. What was live was a Claude Code session (transcript a0c2172d, born 2026-08-26T03:36Z, last write 05:09:48Z, next write 15:59Z under the SAME session id, so the process stayed alive) — idle at the prompt for 29 minutes when the slot said `live` and enumeration said `resumable`; five minutes later both said `live` with no transcript write in between. Had the population held, the falsifier would be met once (target zero): a novel-cause demote of an idle Claude Code session. Out of population: 15 demotes on agents-big-body, 2026-08-25T18:37–20:59Z, every one slot `live` / enumeration `none`, starting eleven minutes after that workspace first appeared in the sweep (18:26:46Z). NOT the trust-gap shape either: it was a codex workspace (codex rollouts naming it 17:34–18:26Z that day; no Claude or Cursor transcripts until 08-31). Cause not established — either the capture-derived scanner fallback in liveness.ts (a capture-less non-Claude seat is enumerated by the Claude scanner and sees nothing) or a slot that outlived its codex process; one test separates them, recorded on ADR 166. No successor watch: the guard belt defers on slotState live, so a five-minute demote of an idle session changes no wake decision; the residual is an enumerator question (why a verdict flips with no new evidence), recorded on ADR 166 (dated note 2026-09-14)."
---
