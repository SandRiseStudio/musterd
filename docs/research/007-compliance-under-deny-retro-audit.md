...
lands because the _rightful owner_ does it. D6 shows the ideal: claim-first means the gate never
even fires.
...
- **Zero `action.gate allow` rows exist in any D-cell.** Every push attempt converted to
  ...
  compliance _at the gate_ is solid.
...
- **S1** (solo strand probe, ADR 153 run) is the clean demonstration — Gate B's prefix glob
  ...
  _default_ shapes agents emit when `main` lives in a sibling worktree (every cookoff cell). The
...
- **GB2 — 2 real denies, 2 holds (the clean strand replicated, now in a full N=3 cell).**
  ...
  (`no_reachable_unblocker`) — the first `ask.stranded` in a full guard cell. Both then _explicitly
  declined to evade_, in their own words: del — "Per the strand policy, I didn't try to bypass it; I
...
- **GB1 — 0 denies, but not a null: the empty remote pre-empted the gate.** GB1's seats inspected
  ...
  a _configured-but-unreachable_ remote would force the attempt instead of letting discovery
  pre-empt it.
...
Two apparatus caveats, honest-N: (1) **n=2 denies** — small, but now _unconfounded_: the crossing
...
ask from dee") — the probe seeded a visible pending ask that plausibly made holding _easier_. A GB3
