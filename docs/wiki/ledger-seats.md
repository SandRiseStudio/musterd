...
Sibling instances found the same night (2026-08-21): `config.modelDrift` — computed, tested, read by nothing; `neverExercised` in the control registry; `ReconcileResult.errors`, discarded by every caller, so a corrupt seat file was silently excluded from the projection. Related: [correct by coincidence](correct-by-coincidence.md), [instrument silence](instrument-silence.md).
