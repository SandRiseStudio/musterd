...
| 5 | `model_source` | `attestedModel()` kept `.model` from `resolveAttestation({model, source})` and discarded `.source` on the next line — then five call sites down-chain had nothing to pass | [#1554](https://github.com/SandRiseStudio/musterd/pull/1554) |
