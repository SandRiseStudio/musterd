...
1. **A declaration where an observation was reachable.** `unknown` was loud and `observed` is right; the case in between said nothing. `shouldWarnUnobservedModel` (in `packages/mcp/src/modelProbe.ts`) is true when the resolved tier is `binding` or `environment` **and** the surface is one whose harness declares an `observeModel` probe — `claude-code`, `codex`, `cursor`, `grok`, `opencode`. The warning names the declared value, the tier and the surface, and prescribes `musterd wire` + a new session.

   The list is **not** in `@musterd/protocol`, though beside `resolveAttestation` is where it first went and where it reads tidiest. Nothing about it is contract-facing — no frame, no schema, no wire value — and `change-adr:check` fired on the path, correctly: a protocol edit needs an ADR because other implementations depend on the wire, and *which of our harness adapters ship a probe* is not something a second implementation can use. Two remedies were on offer (write an ADR, or move it out); the honest one was to move it, because the ADR would have recorded a decision nobody made about a contract that did not change.

   That left the list hand-kept, since the registry owning the truth (`HARNESSES`) is in `@musterd/cli`, which depends on `@musterd/mcp` and not the reverse. So it is pinned from the far side of that edge: `packages/cli/src/onboard/harnesses/probeCapability.test.ts` asserts the list equals `HARNESSES.filter(h => h.observeModel).map(h => h.surface)`. Add a probe without listing its surface and the CLI suite fails — this defect one level up, a probe nobody knows about leaving a seat silently on a snapshot. (Falsify: delete one surface from the list and run that file — 2 failures.)
