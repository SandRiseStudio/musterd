...
## A down daemon rendered as an empty queue, and the seam that separates the two readers (2026-09-14; falsify: with the daemon unreachable, `musterd inbox --waiting` at a terminal must print a line distinguishable from the empty-queue case, and the same command with stdout piped must print nothing) <!-- claim: defect -->
