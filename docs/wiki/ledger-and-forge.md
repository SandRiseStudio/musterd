...
## The two stores do not mirror, and nothing in the repo tries to make them (2026-08-27; falsify: `grep -rln --include='*.ts' -e 'api\.github\.com' -e 'octokit' -e 'gh pr review' -e 'gh pr comment' packages scripts` — measured to return exactly `scripts/submit-dependency-snapshot.ts`; any module that posts a review or comment to the forge means the mirroring exists and this claim is wrong) <!-- claim: defect -->
...
## This page committed its own shape, in the heading above (2026-08-27; falsify: `node -e 'console.log(/\(20\d\d-\d\d(?:-\d\d)?/.test("(adopted 2026-08-27; falsify: x)"))'` — `true` means the wording was in the corpus all along and this is wrong; measured `false`) <!-- claim: defect -->
