...
## The practice change (adopted 2026-08-27; falsify: `sqlite3 ~/.musterd/musterd.db "select id, substr(body,1,200) from messages where act='decline' and from_member=(select id from members where name='ryder') order by ts desc limit 2"` — both bodies cite a GitHub `issuecomment-` id; a substantive decline after this date with no forge pointer means the practice lapsed)
