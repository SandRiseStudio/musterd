...
Corrected 2026-09-24 (lane 01M330A04F, ADR 436 clause 5): superseded by [ADR 434](../decisions/434-a-discharged-act-never-re-wakes-its-seat.md), which reached the daemon at `c8ab98bc` after this run. A recipient's `message` in the act's thread now discharges, and a completed wake spends the act. The half that survives is the out-of-thread reply, which discharges nothing. Ghost did it again on 2026-09-22 10:47 (`request_help` `01M353JW3F`, a directed message with no thread), and it was not re-leased. Since 2026-09-24 the wake line says that only an act IN the thread answers. Falsify with a post-change wake whose seat still answers beside the thread.
...
Fix landed 2026-09-24 by [#1687](https://github.com/SandRiseStudio/musterd/pull/1687) (`fb8cfa6e`, ADR 436 clause 4, lane 01M3309Z28): there were two causes. Nothing stamped a Grok wake ended, **and** the fresh capture was the `wake-<lease>` placeholder, which enumeration never matches, so the ADR 199 same-session override could not fire. The host loop now stamps `ended_at` at settle, and the Grok backend names the real headless session. The falsifier above still stands until a live Grok wake spawns inside the window.
...
Corrected 2026-09-24 (lane 01M3309Y1V, ADR 436 clause 3): "Grok wakes post no capture at all" held only until 2026-09-21 21:03. Every grokbot wake since then posts a `session_captured` row, and every one says `"harness":"claude-code"` (`sqlite3 ~/.musterd/musterd.db "select detail from audit where action='residency.session_captured' and target in ('grokbot','ghost')"`). There were two causes. The hook path's `pushAttestation` never passed a harness, so it defaulted to `claude-code` even when the local slot said `grok`. And the slot itself was inferred from the hook payload's shape, which is Claude's on a woken OpenCode child. The host now stamps `MUSTERD_WAKE_HARNESS` at spawn beside the lease, the capture prefers it, and the push sends the capture's own harness. Falsify the fix with a `session_captured` row for a Grok or OpenCode wake, on a host newer than this change, that still says `claude-code`.
