...
3. **If yes — do NOT stop here.** Check autorefresh's stream for a `bounced the daemon on <sha>` in that window. A deliberate bounce moves `booted_at` exactly as a death does (2026-08-21; falsify: compare `booted_at` either side of any announced bounce — `f588c85` at 14:38:01 against booted_at 14:38:00.362). If a bounce explains the move, the raise is still false. <!-- claim: other -->
...
## `daemon_starved` — six pages in one afternoon were the machine, not the daemon (2026-09-14; falsify: on a laptop at load > 2× cores with the daemon unreachable, the guardian must now record `guardian.observed {class: daemon_starved}` and page nobody — a `daemon_wedged` page under that load means this is wrong) <!-- claim: defect -->
