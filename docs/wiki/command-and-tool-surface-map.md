...
| `nudge` | — | none | the waiting-acts banner and acts, read-only, at the approval prompt (ADR 053) |
...
1. `nudge` → `inbox --waiting` with alias and hook re-point (collision 1). Own lane, small.
