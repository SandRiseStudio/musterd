...
| ~~`join <slug> --as <name>`~~ | — | retired at FEATURE_EPOCH 20; use `claim <name> --team <slug>` (ADR 377) | — |
...
3. **Occupying a seat had three names.** CLI `claim` (from the bound folder), the former CLI `join <slug> --as` alias (naming the Team), and MCP `team_join`. ADR 075 calls the whole thing the *claim handshake*; `requests` decides *claim* requests; and `claim` is also `lane claim` / `lane_claim` / `seed claim`. Falsifier before retirement: `join.ts` and `claim.ts` both ended in the same `POST /claim`. **FIXED:** ADR 377 kept `claim` as the one CLI verb and retired the `join` alias at FEATURE_EPOCH 20. `team_join` remains the MCP tool name; the object-qualified `lane claim`/`seed claim` are fine as they are.
...
5. ~~Seat occupancy gets one name (collision 3). ADR first.~~ DONE: ADR 377 accepted (nick: 2b); every CLI prescription says `claim <name> --team <slug>`, and the former `join` alias retired at FEATURE_EPOCH 20. `team_join` and `claim` name each other.
