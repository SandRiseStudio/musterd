...
Opaque chrome cannot have its contrast reached by the backdrop. **The constraint died the day the opacity change landed and stayed written down for weeks afterward (2026-08-28; falsify: set `--wall` to any colour and run `pnpm a11y:check` — if the constraint still held, the office routes would drop below AA; they measured 14 routes, 0 below AA at both `light=12` and `light=21`).** <!-- claim: defect -->
