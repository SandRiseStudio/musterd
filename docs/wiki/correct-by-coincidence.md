...
- **`multi-harness.test.ts`** was named in [#928](https://github.com/SandRiseStudio/musterd/pull/928)'s body as covering the retired-marker rollout path. Disabling the refusal in `config.ts` leaves it **5/5 green**; the property is really pinned by `surface-drift.test.ts`. Not a defect — but the PR pointed readers at the wrong test, and "pinned somewhere" is not "pinned by the thing named as its acceptance". izzo is correcting it.
- **`speechAddressee`** is well tested as a pure function, while the `OfficeScene.tsx` line that drops the recipient is not: setting `addressee: null` — re-introducing the exact defect seen on the live stream — leaves `packages/web/src/live` **625/625 green** (observed on `e929f160`, 2026-08-21, unmerged; miley may answer this).
