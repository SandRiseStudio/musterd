...
6. **No leftover git conflict markers.** `wiki:check` fails a `<<<<<<<` or `>>>>>>>` line outside a fence (measured 2026-09-15: #1431 landed both into `cloud-seat-from-inside.md` and the gate was green). A lone `=======` is not flagged — it is also a setext underline.
