...
| Hook capture (tool-boundary) | full set: SessionStart/SessionEnd, PostToolUse interrupt (ADR 088), PreToolUse gate (ADR 150) + observer (ADR 167), Notification | same as desktop (hooks are harness-level, driver-independent) | same, where the harness runs at all | sessionStart/postToolUse/sessionEnd — **no PreToolUse gate, no interrupt** | IDE set **plus** ADR 265's `afterShellExecution`/`afterMCPExecution` — added because the CLI did not dispatch the ADR 198 set (2026-08-13) | SessionStart/SessionEnd/PostToolUse only (`.codex/hooks.json`); no gate, interrupt, or notification hooks |
