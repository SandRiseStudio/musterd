...
always `orient-stamp`. The peer sweep was retired 2026-09-24 by ADR 442 (#1676). `label-nudge` is silent when `TERM_PROGRAM` is a known terminal emulator.
...
| Hook drift detection | `inspectGrokHookDrift` populates `hookDrift`. ~~marker presence only (2026-09-02)~~ HOLDS 2026-09-14: exact command-text + `FEATURE_EPOCH` two-way (ADR 168). A same-marker discarded-stdout PreToolUse is STALE; a newer epoch blames the checkout (falsify: a same-marker interrupt hook that discards stdout passing `inspectGrokHookDrift`) | same | <!-- claim: other -->
