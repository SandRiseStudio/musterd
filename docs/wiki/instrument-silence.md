...
## An Eval nobody computes is a write-only instrument (2026-09-02; falsify: `docs/wiki/adr-180-reviewer-eval.md` — find a reader of the reviewer's comments before that page)

ADR 180's advisory PR reviewer posted findings on 22 of 80 PRs for five weeks and got zero replies (2026-09-02; falsify: the per-PR loop on that page); its own pre-registered Eval said "near zero ⇒ drop it" and was never run. Fourth of its kind in a fortnight (ADR 250 repeat-wake, ADR 338 drift, ADR 252 floor). The tell is the same as the tools above: an instrument that writes and is never read is not silent, it is unheard — and the fix was to read it once, not to make it louder.
