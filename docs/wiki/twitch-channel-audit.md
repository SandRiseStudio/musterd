...
| Videos / past broadcasts | ~~**Empty.** "Recent broadcasts" and "All videos" both show nothing — VOD storage is off, so no session survives its own end.~~ **WRONG, corrected the same day** — the archive held **30 past broadcasts**, several predating any setting change. See [the correction](#vod-storage-was-already-on-and-the-empty-archive-was-a-carousel-that-had-not-rendered-2026-09-16-falsify-open-the-videos-page-and-count-the-cards-rather-than-reading-the-headers). | The only real gap here was **clips**, not recordings. |
...
1. **Cut clips, and keep cutting them.** ~~"Turn on VOD storage, and clips. Every session produces nothing that outlives it" (2026-09-16)~~ HALF WRONG, corrected the same day (2026-09-16; falsify: the Clips tab lists a clip of our own): storage was already on and 30 recordings existed. What was genuinely empty was clips — the tab held only the prior owner's seven. Recordings expire in 7 days on this channel and clips do not, so the clip is the artifact and the recording is only the window in which one can be cut. Live concurrents for a niche engineering stream stay small whatever the category; a clip is the thing that travels off Twitch, which is where the audience actually is (see [below](#the-part-that-is-not-a-twitch-setting-2026-09-16-falsify-after-the-settings-above-land-compare-the-referrer-mix-on-the-watch-page-and-the-channels-own-analytics--if-twitch-directory-browse-is-a-majority-source-this-reading-is-wrong)). <!-- claim: other -->
...
## VOD storage was already on, and the "empty archive" was a carousel that had not rendered (2026-09-16; falsify: open the Videos page and COUNT THE CARDS rather than reading the headers) <!-- claim: defect -->

The headline finding at the top of this page was wrong, and it is worth more than a struck line because a real action was taken on it.

**What is actually there**, read at ~14:10 PDT: **30 past broadcasts**, the oldest sampled from **17 hours earlier** — long before any setting on this channel was touched. Storage had been capturing all along.

**How the mistake was made.** The first reading scraped the Videos page and got back `Recent broadcasts | View All | All videos | View All` with no cards between them. Those headers render before their content does. An empty-looking carousel was read as an empty archive, written down as "VOD storage is off", and never re-run — even while this same page was being edited four times.

**What it cost.** Two things, and neither was hypothetical:

- **A deliberate three-minute outage.** The stream was stopped and restarted in front of viewers on the reasoning that the running session was not being captured. It was. The restart bought nothing. Worse, `stream start` then failed on a missing image digest, so the channel was dark while a 594 MB image built — `musterd help stream` says `doctor` "is the one to run first" and it was run after the stop.
- **The leverage order.** "Turn on VOD storage" was ranked the single highest-return action on this channel. It was already done.

**What survives the correction.** The clips half was right: the Clips tab genuinely held nothing of ours. The [7-day retention](#a-stored-vod-is-not-a-durable-artifact-it-is-deleted-after-7-days-on-this-channel-2026-09-16-falsify-twitchs-on-demand-content-help-article-states-the-retention-tier-for-each-account-type) facts are unchanged and now matter more, not less, because there are 30 recordings quietly aging out rather than none.

**The rule this adds to the two already on this page.** An absence read off a rendered page is only a finding if the page was given a chance to render, and the check must count what is present rather than read what frames it. This is the third instrument failure recorded here in one day — a GraphQL query that ignored its filter, a claim carried forward without re-running, and now a header mistaken for its content. All three had the same shape: **something answered, and the answer was believed because it arrived.**

## The archive is 30 near-identical stubs, which the empty-archive belief hid (2026-09-16; falsify: open the Videos page and read the durations and titles down the list) <!-- claim: defect -->

Seeing the archive for the first time surfaced a defect nobody could have found while believing it was empty. The 30 broadcasts are mostly **under 25 minutes** — sampled durations include 3:10, 5:01, 5:53, 8:57, 14:00, 14:20, 15:39, 17:58, 20:57, 24:02, with one 46:21 — because the stream restarts constantly: supervisor heals, deploys, and at least one deliberate stop. Every one carries the **same title** and **0 views**.

A stranger opening Videos therefore sees three dozen indistinguishable stubs rather than a body of work, which is worse than two clean recordings would be.

**The cause is restarts, not titling, and the two fixes are not interchangeable.** Measured 2026-09-16: the auto-refresher alone bounced the daemon **15 times that day**, before supervisor heals and one deliberate stop/start. Titles make the fragments *legible*; they do not make them *fewer*. Reducing the count is a stream-restart policy question on the broadcast rail ([broadcast-stream](broadcast-stream.md)) and is deliberately not taken here. Making them legible is copy, and is specified as a per-working-session title in [the channel copy spec](../design/twitch-channel-copy-spec.md) §3.1 — sticky across restarts, so one title covers every recording inside a session and a human is never asked to type thirty a day. Two directions, neither taken here: fewer and longer broadcasts (a stream-restart policy question, and the broadcast rail's lane, not this one), or per-session titles so the archive reads as a sequence rather than a repetition — the title is set in Edit Stream Info and past broadcasts inherit it, so a title changed per session is inherited per session.
