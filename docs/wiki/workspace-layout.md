...
**Trap — a member Workspace must be its own git root.** A plain folder at `~/musterd/<team>/<repo>/<member>` has no `.git` of its own, so git walks up to the roster repo at the team root, and Claude Code (which keys the musterd MCP entry by the repo root it finds — ADR 165) then loads **no musterd tools** for that seat. `claude mcp get musterd` from the folder is the one-line check: a healthy seat shows the entry connected at `~/.musterd/runtime`. Found 2026-09-25 on fifty, created by `musterd agent --path` as a plain folder before `--path` learned to make a worktree; `musterd doctor` now names the case and prints the repair (add a worktree beside it, `rsync --ignore-existing` it in, `git worktree repair`) — which leaves `.claude/` and `.musterd/` in place, so it is safe under a live session, which then needs a restart. Falsifier: a seat folder under the layout whose `git rev-parse --show-toplevel` is not itself, with the musterd tools loading anyway.
