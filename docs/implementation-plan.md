...
- **ADRs 037–041** — the public **web surface** (037: marketing + roadmap map + the winding-road visual); the **role-label↔template** unification (038); **cross-network teams** — the topology framework decided (039) and the **secured off-loopback bind** built (040: refuse a non-loopback *plaintext* bind, native TLS/`wss://` or `--insecure-trust-proxy`, Origin/Host gate on the WS upgrade, env-tunable WAN timeouts) plus the Topology B overlay operator guide; and the **roadmap single-sourced** to typed data with `ROADMAP.md` generated from it (041). The transport that lets the daemon leave localhost now exists — the v0.3 credentialed remote join it carries is still unbuilt.
