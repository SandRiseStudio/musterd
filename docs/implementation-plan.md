...
>
> Last reconciled: **2026-06-12** (v0.2 **M4 docs promotion** in progress — `SPEC.md` promoted to `musterd/0.2`, `03/04/05` architecture docs updated for the dormant-by-default adapter / six tools / shutdown contract / single-active + activity / intent-led `init`, and the *team = standing roster* decision recorded in the canonical glossary. M3 core + tail landed earlier via PR #1.) **Only open M3 item: the `provenance`/`where`-on-attach seed.** **M4 tail remaining:** flagship demo + Scenario C (refused duplicate + `working` in watch pane), `.gitignore`/secret-config warning.
...
4. **M4 — docs promotion + flagship update.** 🟡 **in progress — docs landed (this session); flagship/demo updates remain.** **Done (this session's PR):**
   ...
   - **Still open (M4 tail, follow-up PR):** `examples/flagship-demo.mjs` + Scenario C (show one refused duplicate + `working` in the watch pane); README quickstart already reflects explicit activation (PR #2); `.gitignore`/warn on secret-bearing harness configs.
