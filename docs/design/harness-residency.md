...
| **codex desktop**           | adapter/project MCP attach; no stable host-owned task capture API                   | manual resume only                                                            | project MCP reload + ordinary coordination evidence | not daemon-wakeable unless the ADR 204 stable-API probe passes; durable inbox delivery remains available                                  |
