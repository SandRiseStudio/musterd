...
| **codex CLI**               | `exec --json` → typed `thread.started.thread_id`, stored only in the local binding | exact `exec resume <id>` → one fresh `exec -C <workspace>` fallback          | fresh wake-provenance roster Presence **+ thread-id match** | resume has no `-C`, so child cwd is the workspace boundary; a clean exact resume without fresh Presence fails rather than launching again |
| **codex desktop**           | adapter/project MCP attach; no stable host-owned task capture API                  | manual resume only                                                           | project MCP reload + ordinary coordination evidence         | not daemon-wakeable unless the ADR 216 stable-API probe passes; durable inbox delivery remains available                                  |
...
| **grok CLI**                | SessionStart/SessionEnd → `musterd session … --stdin`; enumerate `summary.json`    | `grok -r <id> -p` → fresh `grok -p`                                          | roster occupancy                                            | no `--yolo` on wake (ADR 352); stdio MCP must disable+enable to respawn                                                                 |
...
The Codex desktop row is deliberately weaker than the Codex CLI row. The desktop MCP adapter proves
coordination, not host actuation: a manual app reopen or `/mcp reload` cannot establish exact task
targeting, lifecycle observation, or safe wake/resume. The repeatable evidence procedure is
[`tests/codex-desktop.md`](../../tests/codex-desktop.md); its result must not be used to register a
