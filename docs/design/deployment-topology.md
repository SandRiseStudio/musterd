...
For the public phone-MCP demo, the separately governed Cloudflare Tunnel route is an exact-path
variant of Topology B. It uses a dedicated daemon authority and persistent ordinary Team state; it
does not change the private-overlay recommendation for ordinary cross-network Teams. The public
edge and loopback trust boundary are recorded in [ADR 448](../decisions/448-public-demo-tunnel-route.md);
the host, lifecycle, and operator procedure are recorded in [ADR 451](../decisions/451-public-demo-ordinary-members.md)
and the [public-demo runbook](../operations/public-demo-tunnel.md).
...
| 3c | Hub-authoritative claim CAS, seat→node residence binding | landed 2026-09-02 (ADR 355): a joiner's self-claim is decided by the hub's guarded CAS; refusals name the holder; an unreachable hub refuses with `hub_unreachable`. Residence is a hub-minted `seat_nodes` binding (v59, ADR 355 §5 amendment after gptbot's review): first node to claim as a seat holds it, a claim for a seat bound elsewhere is `403 bound_elsewhere`, an admin unbind is the explicit re-bind act; the `node` on `lane.claimed` is the trace. Push-level residence (ingest checks the seat, not just the team) landed for presence in 3d and for every kind in [ADR 360](../decisions/360-push-level-residence.md) (2026-09-02; the hub never refuses its own loopback, service seats are resident everywhere). Presence replication is 3d. Since [ADR 358](../decisions/358-human-seat-trusts-a-set-of-machines.md) (migration v63) a human seat's binding is a set, widened by `musterd node trust` from a node already in it; agents stay one-node |
| 3e | Every ownership edge behind the hub — release, handoff, submit, close via `/sync/lane`; the origin speaks | landed 2026-09-02 ([ADR 361](../decisions/361-every-ownership-edge-is-the-hubs.md)): one predicate (`owner_seat` or `state` on the patch), one shared policy, the joiner's read as the CAS; `node` on every arbitrated `lane.*` row; the hub's duplicate `[lane] claimed` act removed. Field edits stay offline-capable |
