...
**Every row above is backed by a daemon-side delivery row, and has been since 2026-07-05.**
`GET /inbox/interrupt-check` writes an `interrupt.raised` audit row per (recipient, act) — actor =
sender, target = recipient — from ADR 088 increment 1 (#109). The laptop daemon held 253 of them on
2026-09-19, spanning 2026-07-06 onward. The "measured" column therefore names *who looked*, not the
only evidence that exists: for any of these measurements the row is still queryable after the fact,
which is why a doorbell eval is a query rather than a race against its own PostToolUse hook.

Two limits, both real. **Before ADR 423** the row carried no `rail` and no verbatim `line`, so it
could prove *that* a seat was rung and not *what with* or *through which seam* — which is why this
table was built on measurers' names in the first place, and why rows written before 2026-09-19 have
an absent rail meaning "not recorded" rather than "no rail". And no daemon-side row can prove the
harness **surfaced** the line it was handed; that needs the harness's own transcript, which is what
each measurement in the last column actually went and got. The verdicts stand on that, as they
should.
...
| harness | verdict | what was measured |
| --- | --- | --- |
| claude-code | **defers, re-defers, and can mute outright** | musterd tools arrive deferred; schema fetched via `ToolSearch` (stanley, izzo, ryder). Reconnect has TWO measured outcomes, and the difference is a tax versus an outage. (a) Schema cache dropped, every schema fetched again with no permission error — the seat pays a round-trip and works (ryder, 2026-09-14). (b) A mid-session MCP drop after which every tool reports "no longer available … ToolSearch will return no match" and `/mcp` fails with `CONNECT_TIMEOUT`: **no in-conversation recovery, 65 min observed**, while grant, credentials, presence and daemon all stayed healthy and the CLI channel answered throughout — i.e. the Cursor shape, on claude-code (dolly, session `8a006b8a`, 2026-09-14; `docs/wiki/claude-code-live-doorbell-eval.md`). What decides between (a) and (b) is unmeasured |
...
- **Clause 8 — lane `01M2GP2Z90` (schmidt, Cursor evaluation landed):** Cursor measured live. Dynamic
  ...
  Claude-code defers and re-defers **and can also mute outright** — the same permanent-mute shape,
  measured from inside the failure on a second claude-code seat (dolly, lane `01M2GYJX55`, #1420);
  what decides between the recoverable and the permanent outcome is still unmeasured.
  Grok defers behind `search_tool`/`use_tool` (reconnect unmeasured);
