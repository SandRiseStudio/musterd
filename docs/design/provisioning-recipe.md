...
> **Status: Phase 1 + Phase 2 (local) built.** Phase 1 = "roles provision tools" (the Role JSON template, built-in seed library, Claude Code/Cursor/Codex MCP-server provisioning, uninstall manifest, charter injection, init wiring; ADRs 029–031). **Phase 2 = the local claim-on-first-use half of §5–§6** — the folder claim policy (`MUSTERD_CLAIM`), the overloaded `team_join`, `musterd claim`, and client-side pending presence — built locally with **no wire change** (ADRs 032–033). The **governed** half (agent key + grants + the approval lane + the A.3 `claim` handshake) is still design, v0.3-gated, activating when the daemon leaves localhost. The detailed design is under **ADR 026** (harness tool environment / two universes), governed by **ADR 027** (non-invasive coexistence) and **ADR 028** (compose, don't capture). The wire-level seat/claim handshake is `SPEC.md` Appendix A.3; the governance/auth model (seats, grants, capabilities) is `membership-model.md`. **This doc owns** the *provisioning recipe* (what a Role template is + how it renders into a harness) and the *local onboarding/claim experience* (`init`-once, claim-on-first-use).
...
**Built (Phase 2 — local claim-on-first-use, ADRs 032–033):** the **folder claim policy** (`MUSTERD_CLAIM` grammar — `chat` / `seat:Ada` / `role:backend` — resolved via the ADR 018 env→binding ladder; `init` stamps `seat:<name>` *alongside* the minted identity, so re-init is never needed to add a teammate); **`BindingSchema` carries an optional identity** (`member`/`token` now optional) + the policy, so a folder can be bound to a policy with no fixed seat; the **overloaded `team_join`** (`{as}` named / `{role}` next pool handle / `{}` policy — the result returns the assigned identity, and the charter lives in `AGENTS.md`); **`musterd claim <name>` / `--role <x>`** — the L2 universal floor, mint-or-reuse → write the binding; **conflict semantics** (own reload → newest-wins reclaim via the held token; a name another session holds → `claim_conflict` with the roster + a fresh-name/`--role` hint; free/in-grace → re-occupy) — all riding today's `hello`/members primitives, **no wire change** (`SPEC.md` A.3 stays Unreleased); **client-side pending presence** — an unclaimed binding is reachable but holds no seat, `team_send`/`team_inbox_check` refuse with a claim hint, and `.musterd/pending/<code>.json` markers let `musterd claim --for <code>` disambiguate several waiting sessions; **`musterd claim --for <code>` also brings an *already-running* pending session online** (ADR 034) by dropping a 0600 `<code>.resolved.json` sidecar the adapter's resolution watcher adopts, no relaunch needed. **Locally, claiming auto-mints** the seat via the unauthenticated `POST /members`; the grant/approval governance layers on only off-localhost.
