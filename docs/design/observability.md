...
- Observability of agent internals (reasoning steps, LLM calls) — that's the existing market's job; we link to it, we don't replicate it. _Scoped 2026-09-24 ([ADR 445](../decisions/445-agent-traces-captured-local-first.md)): this stays the product boundary. Under ADR 082's dogfood line, the daemons we run additionally capture their **own** seats' harness activity (tool calls with inputs/outputs, prompts, reasoning, usage, hook outcomes, turn/subagent boundaries) local-first into a separate `trace.db` as research substrate — structural by default, content opt-in per team with a credential scrub, never replicated or exported without ADR 184 consent. No spans database or LLM-call dashboard for other people's traces; nothing ships off-machine by default._
