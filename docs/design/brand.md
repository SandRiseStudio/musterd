...
What the gate enforces is narrower than what this table publishes, and the difference is deliberate. Linted outright (no legitimate second sense): **kit**, **profile**, **template**, **worktree**. <!-- vocab:ok --> <!-- this sentence IS the gate's own definition, and `lintedSetDrift` parses these bold spans to prove it has not drifted from `terminologyBans()` — so they cannot be backticked into mentions, and the suppression above is the only way both checks can be true at once --> Every other word in a Not column is authorial guidance, held in review rather than by a regex: those words are wrong only when used *as* the canonical term, and `adapter` the MCP adapter, `connection` the WebSocket, `participant` in a huddle and `verb` in the audit log are all correct English in this repo. A gate that banned them outright would buy suppressions, not clarity ([ADR 296](../decisions/296-terminology-architecture.md), amended 2026-09-05).
