...
- It is a real framework emitting exactly the per-agent gen_ai spans batond links _between_ — proving "completes, doesn't fight" with a third party, not a strawman.
...
3. **Replay safety on triggers.** External triggers (webhooks from GitHub/Stripe/Slack, memory-state changes) carry signature verification and **idempotency keys**, so a redelivered event cannot double-fire an agent. musterd's wake surface is growing the same shape of inbound edge — residency wakes (ADR 131), raised-deferral wakes (ADR 211), the interrupt line — and the ADR 210/220 delivery work is already tightening wake _accounting_; an idempotency key on wake/act delivery is the cheap complement that makes redelivery safe by construction rather than by audit.

4. **Cost/turn budgets as a first-class knob.** AgentField attaches spend and turn budgets to execution. musterd _measures_ cost well (wake_cost, the cookoff waste numbers, `report coordination`) but _bounds_ nothing. A soft per-lane budget — declared at `lane_open` ("this is a ~$5 unit of work"), compared against attested spend, surfaced as an advisory warning at the threshold — fits warn-never-block, gives the steward and the human a drift signal mid-flight instead of post-hoc, and feeds the economics musterd already publishes. Deliberately _not_ enforcement: a budget that halts a seat is the control-plane move we just declined to copy.
