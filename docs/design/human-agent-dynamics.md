...
> **Status: PHILOSOPHY, not spec.** Nothing here is buildable as written; it is the conceptual ground under decisions other docs make buildable. It came out of the first real dogfooding session (2026-06-11, the day v0.2 M1 landed): a human driving an agent through musterd noticed the roster showed the agent online and *himself* offline, and pulled the thread. Where this note proposes anything concrete it is flagged as a **design seed** with the roadmap item it belongs to. Companions: `research-foundation.md` (the *empirical* ground — the Co-Gym result this philosophy operationalizes), `membership-model.md` (v0.3 governance — the *enforced* layer below), `docs/archive/membership-impl-plan.md` (v0.2), `../decisions/007-v0.2-scope-cut.md`.
...
1. **Provenance** *(fact, cheap, objective)* — why does this presence exist? A human opened a harness session; someone asked the agent to do something; a hook/function/app fired; a schedule started it ("it's 9am"); it's a 24/7 daemon. Knowable at attach time, never guessed. → **✅ SHIPPED (v0.2, ADR 014): a `provenance` enum on attach** — `session | asked | hook | scheduled | daemon`. It dissolves most of §1 without modeling humans at all: `Tim — online via claude-code (session)` already tells the team "someone is behind this," vs. `(scheduled)` = "nobody necessarily is." The MCP adapter sends it from `MUSTERD_PROVENANCE` (default `session`); the CLI's `inbox --watch` attaches as `session` (the supervising posture).
...
**✅ SHIPPED (v0.2, ADR 021): driver co-presence.** The adapter knows when it is running inside a human-initiated session, so it now sends an optional `driver` (the human's name) on `hello`, and the roster renders `online via claude-code (session) · driven by nick` instead of showing the driving human offline. The Co-Gym dual-control finding (`research-foundation.md`: humans co-act 21–32% of the time — they are participants, not spectators) promoted this from "nice signal" to "the roster is lying about a measured participant," and it was the pre-launch gate for the real 3-pane demo (`../implementation-plan.md` §4.A0). **Mechanism settled (ADR 021): the middle path** — a `driver` label on the agent's *own* presence (mirroring how ADR 014 added `workspace`), *not* a second presence row attributed to the human. A paired human row (Option A) was rejected because the adapter holds only the agent's token and must never impersonate the human or hold their identity; naming the driver on the agent's presence makes the roster truthful without that. `init` bakes the operator's name into the agent's MCP env (`MUSTERD_DRIVER`) so the demo works out of the box. A *real* paired human presence, with proper member linkage, remains the v0.3 governance surface.
...
- ~~**M3 candidate:** the `provenance` enum on attach~~ **✅ shipped (v0.2, ADR 014)** — provenance *and* the `where`/workspace half landed together: optional `hello` fields, recorded on the presence, rendered dim on the roster.
