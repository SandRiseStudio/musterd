...
6. **Capability-scoped, need-to-know.** A seat may do only what its role's capabilities allow (comms, tools, declared resource scopes, visibility). For **credentials, grants, audit, team policy and other roles' charters**, a seat sees only what it needs: admins see all; non-admins get a viewer-scoped projection. For **acts**, what a seat needs to see is what its teammates are doing — its own next act is a claim about that — so every claimed seat reads every act on its team, save the named class in ADR 407 §3 (amended 2026-09-16, ADR 407).
...
- **Need-to-know visibility:** roster/info endpoints return a **viewer-scoped projection**. Non-admins never see credentials, grants, audit, team policy, or other roles' charters — only teammate handles and presence. **Acts are team-wide** (ADR 407): visibility and delivery are separate axes. A seat is _sent_ what is addressed to it, the team, or an eligible set it is in (the inbox, `--waiting`, the doorbell, the cursor); it may _read_ every act on its team.
  > **Shipped (ADR 128/136, amended by ADR 407 — 2026-09-16).** `GET /teams/:slug/messages` and the `team-all` firehose apply one visibility grade per reader (`messageVisibilityOf`): **full** (admins, full-grade observers) reads every envelope; **team** (every other claimed Member) reads every envelope save a to-human `ask` carrying `meta.about` that names a seat and is not the reader's own; **public** (a shared watch-link observer) reads Team-broadcast traffic only. Reading is not receiving: a directed act read via visibility never enters the reader's inbox, moves its cursor, rings its interrupt line, or discharges an eligible set. Watch links are not anonymous: they still expose roster handles and Presence.
...
- **Agent bootstrap credential:** an admin mints an independently revocable `mskey_` for one seat, declared role pool, or residency host. The plaintext is shown once; only its hash and redacted inventory metadata remain. Rotation is staged: mint a successor, distribute and verify it, then revoke the predecessor. A marked legacy Team-wide key remains only during that Team's compatibility window: each Workspace can exchange it for seat scope, readiness requires successful scoped use by every held agent seat and enrolled host, and an administrator then disables it transactionally. This per-Team retirement does not remove the compatibility code; product-wide removal waits until every extant Team has cut over ([ADR 350](../decisions/350-retire-legacy-team-bootstrap-key.md)).
