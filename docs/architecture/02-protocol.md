...
- `TraceEvent` / `TraceEventBatch` / `TraceIngestResponse` (`trace.ts`, ADR 445 R1) — the hook tap's structural event: `harness`, `session_digest`, `ts`, `kind` (`TRACE_EVENT_KINDS`: the Claude Code hook events plus `HookOutcome`), optional `tool_name`/`tool_use_id`/`agent_id`/`parent_agent_id`/`duration_ms`/`outcome`/`detail` (scalars, ≤1 KiB). No field for tool input, response, prompt or reasoning exists in this schema; increment 1b adds the content part behind the credential scrub. `seq` is not on the wire — the daemon assigns it.
