...
| `POST` | `/teams/:slug/members/agents` | `{ "name","role?" }` | `{ "member","connect_url","connect_expires_at" }` | a **human** member mints an agent they sponsor (ADR 449 §3): `sponsored_by` = caller, `lifecycle_until` = the caller's, ≤3 live per sponsor; no secret in the response — a one-time `/join/:team#n=<nonce>` connect link (15 min). Refused on a file-backed team (seat files are the writer, ADR 058) |
| `POST` | `/teams/:slug/members/agents/:name/connect` | — | `{ "member","connect_url","connect_expires_at" }` | the sponsor re-issues the connect link; the prior unused link is burned (ADR 449 §4) |
...
| `GET`  | `/teams/:slug/members/me/doorbell`           | —                                                                   | `{ member, prefs, allow, defaults, team_urls, route }`            | a human's own doorbell sinks, personal URLs included; `team_urls` says only whether the team set each URL; agents 403 (ADR 443) |
| `PUT`  | `/teams/:slug/members/me/doorbell`           | `{ "sinks": { <sink>: { on, tiers?, url?, host? } } }`              | same as `GET`                                                     | set your own sinks; 422 for a sink outside the team allow-list or a URL that is not `https` to a public host — the message names the sink, never the URL (ADR 443) |
| `GET`  | `/teams/:slug/members/:name/doorbell`        | —                                                                   | `{ member, sinks: { <sink>: { on, personal } }, route }`          | admin-only; masked — says a personal URL exists, never what it is (ADR 443 §5) |
| `POST` | `/teams/:slug/doorbell/rings`                | `{ "host" }`                                                        | `{ rings: [{ id, member, record }] }`                             | the host claims the `os` rings queued for its label (wake-lease auth: team agent key or a credential scoped to that host); each ring is handed out once, and a ring answered, past its deadline or over an hour old is claimed without being returned (ADR 443) |
| `POST` | `/teams/:slug/doorbell/rings/:id/surfaced`   | `{ "host","ok" }`                                                   | `{ ok }`                                                          | the host reports a banner; writes `doorbell.surfaced {surface:"os", ok}`; 404 for a ring not queued for that host; 409 once reported or when it was claimed but not returned; `ok` is the notifier's exit, not the call (ADR 443) |
| `POST` | `/teams/:slug/trace/events`                  | `{ "events": TraceEvent[1..256] }`                                  | `202 { "accepted", "content" }`                                 | the trace tap's ingest (ADR 445 R1 hooks + R2 transcript records): seat credential, leaseless, presence-neutral; the seat is the caller, never a body field; a content part is stored only under `trace.content: on`, and the reply's `content` is how a tap learns that mode                                                                                                        |
| `GET`  | `/teams/:slug/report/trace`                  | `?days=N` (1..90, default 7)                                       | `TraceReport`                                                   | ADR 445 increment 4: coverage / tool mix / cost per lane from structural trace columns; ADR 128-scoped (own seat, or every seat for an admin)                                                                                                       |
| `GET`  | `/teams/:slug/trace/sessions/:digest`        | —                                                                   | `{ events }`                                                    | one session's trace in seq order (ADR 445 increment 2, behind `musterd trace show`); ADR 128-scoped: own seat or admin, everything else reads empty                                                                                                        |
...
- `TraceEvent` / `TraceEventBatch` / `TraceIngestResponse` (`trace.ts`, ADR 445) — the trace tap's structural event: `harness`, `session_digest`, `ts`, `kind` (`TRACE_EVENT_KINDS`: the Claude Code hook events plus `HookOutcome` — rail R1 — and, lowercase to keep the rails apart, the rail R2 transcript records `reasoning`/`assistant_text`/`usage`/`unknown`, increment 2), optional `tool_name`/`tool_use_id`/`agent_id`/`parent_agent_id`/`duration_ms`/`outcome`/`detail` (scalars, ≤1 KiB), and (increment 1b) an optional `content` part — `TraceContentSchema`: `prompt`/`tool_input`/`tool_response`/`error`/`assistant`/`reasoning` strings (the last is R2-only — no hook carries it), bounded together to `TRACE_CONTENT_MAX_BYTES` (256 KiB), with `redactions` and `truncated`. `seq` is not on the wire — the daemon assigns it. `TraceIngestResponse` carries `accepted` and the team's `content` mode (absent from a pre-1b daemon, read as `off`). `TracePolicySchema` (`{content: off|on}`, default `off`) is `policy.trace`. `TRACE_POLICY_FILE` names the workspace's cached mode; `TRACE_TAIL_FILE` the R2 offset-cursor file beside it. `TraceSessionEventSchema` / `TraceSessionResponseSchema` shape the `GET …/trace/sessions/:digest` read — deliberately looser than the write schema, because a reader renders whatever history a past writer stored.
- `TraceReportSchema` (`traceReport.ts`, ADR 445 increment 4) — what `GET /teams/:slug/report/trace` returns: `coverage[]` per harness (sessions, sessions_with_r2, post_tool_use, r2_tool_uses, coverage ratio or null), `tool_mix[]` (harness, tool, calls, errors, avg_duration_ms), `lane_cost[]` (lane, title, state, seat, goal_id, turns, input/output/cache_read tokens) and `unattributed` — with `TRACE_REPORT_DEFAULT_DAYS` 7, `TRACE_REPORT_MAX_DAYS` 90, `TRACE_TOOL_MIX_TOP` 15.
- `SyncTraceRowSchema` / `SyncTracePushRequestSchema` / `SyncTracePushResponseSchema` (`traceSync.ts`, ADR 453 §2) — the structural-only trace row on the joiner → hub wire, typed so prose cannot fit: ULID `id`, seat/harness patterns, closed `kind`/`outcome`, 24-hex id digests (`TraceIdDigestSchema`), counts, and a per-kind closed `detail` (`traceStructuralDetailSchema(kind)`, from `TRACE_DETAIL_KEYS_BY_KIND`: count / flag / enum / model-id types, the producer allowlists `TRACE_HOOKS`, `TRACE_REASONS`, `TRACE_PARSERS`, `TRACE_UNKNOWN_TYPES`, …). `tool_name` is bounded to the harness namespace (`TRACE_TOOL_NAMESPACES` + `TRACE_MCP_TOOL_RE`) or the sentinel `TRACE_OTHER`; `structuralToolName` / `normalizeTraceDetail` are the normalizers the pusher and local ingest run (never reject), `isStructuralToolName` the check the hub runs (reject). `TRACE_STRUCTURAL_DETAIL_KEYS` is the union `dataset:export` allowlists — one definition of "structural" for the dataset and the channel. The credential scrub runs on every string field at the wire.
- `scrubCredentials` / `scrubToText` / `redactionMarker` (`traceScrub.ts`, ADR 445 §3) — the credential scrub, run at the hook and again at ingest: every `TOKEN_PREFIXES` token (registry-driven, no start boundary), PEM private keys, `Authorization` values, bare `Bearer` tokens, `sk-`, GitHub, AWS, Slack, and 32+-char values assigned to a token/key/secret/password name; each becomes `<redacted:kind>` and is counted. The start boundary accepts JSON (`\n`, `\uXXXX`) and URL (`%XX`) escapes. `scrubToText` scrubs a structured value's string leaves before stringifying it. Idempotent. Credentials only — not a PII scrubber (ADR 184 §2).
