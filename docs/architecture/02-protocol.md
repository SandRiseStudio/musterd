...
| `POST` | `/teams/:slug/trace/events`                  | `{ "events": TraceEvent[1..256] }`                                  | `202 { "accepted" }`                                            | the hook tap's ingest (ADR 445 R1): seat credential, leaseless, presence-neutral; the seat is the caller, never a body field; structural columns only — the schema has no content part                                                                                                        |
...
- `TraceEvent` / `TraceEventBatch` / `TraceIngestResponse` (`trace.ts`, ADR 445 R1) — the hook tap's structural event: `harness`, `session_digest`, `ts`, `kind` (`TRACE_EVENT_KINDS`: the Claude Code hook events plus `HookOutcome`), optional `tool_name`/`tool_use_id`/`agent_id`/`parent_agent_id`/`duration_ms`/`outcome`/`detail` (scalars, ≤1 KiB), and (increment 1b) an optional `content` part — `TraceContentSchema`: `prompt`/`tool_input`/`tool_response`/`error`/`assistant` strings, bounded together to `TRACE_CONTENT_MAX_BYTES` (256 KiB), with `redactions` and `truncated`. `seq` is not on the wire — the daemon assigns it. `TraceIngestResponse` carries `accepted` and the team's `content` mode (absent from a pre-1b daemon, read as `off`). `TracePolicySchema` (`{content: off|on}`, default `off`) is `policy.trace`. `TRACE_POLICY_FILE` names the workspace's cached mode.
