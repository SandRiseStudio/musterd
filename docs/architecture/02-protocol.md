...
| `POST` | `/teams/:slug/doorbell/rings/:id/surfaced`   | `{ "host","ok" }`                                                   | `{ ok }`                                                          | the host reports a banner; writes `doorbell.surfaced {surface:"os", ok}`; 404 for a ring not queued for that host (ADR 443) |
