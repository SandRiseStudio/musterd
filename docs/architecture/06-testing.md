...
- `musterd` (cli) + `@musterd/mcp`: **≥ 75% lines** (the command/tool dispatch and error→exit mapping must be covered) — **enforced**, met (cli ~78%, mcp ~96%). The former gap — the interactive onboarding wizard (`cli/src/onboard`) and the MCP tool handlers (`mcp/src/tools`) — is now covered by behavioral tests that drive `runInit` with `@clack/prompts` mocked and call each tool handler against a stub client. The floors only ever move up — ratchet them when coverage rises (mcp went 57→64 with the OTel-propagation tests, then to 75 with the tool-handler tests).
- Root `pnpm test` runs unit+integration+scenario and must be green for any milestone to be "done" (`07-conventions.md` definition of done); `pnpm coverage` additionally enforces the gates above.
- **CI is the authoritative gate** ([ADR 104](../decisions/104-unified-git-workflow.md)): `.github/workflows/ci.yml`, job **`gates`**, runs `pnpm install` → `pnpm build` (workspace packages resolve to gitignored `dist/`, so build precedes) → `pnpm typecheck` → `pnpm test` → `pnpm format:check` on every PR, and branch protection **requires it green to merge**. Use root **`pnpm test`** (not `pnpm -r test` — that trips a cwd/include quirk in `@musterd/telemetry`). Tests must be environment-stable: CI sets `CI=1` (color libs emit ANSI) and checks out a detached HEAD, so `vitest.config.ts` pins `NO_COLOR=1` and env-dependent assertions must not rely on ambient TTY/git/branch state. Coverage (`pnpm coverage`) is **not** in the `gates` gate today — it stays a local DoD check.
