...
- `@musterd/server`: floor **≥ 82%** (**ratchet goal 85**), with route/presence/inbox/auth paths covered by integration tests specifically. It had drifted to ~82 while no CI ran coverage; the floor freezes that and the gate stops any further drop.
- `musterd` (cli): floor **≥ 73%** (**ratchet goal 75**) — the command/tool dispatch and error→exit mapping must be covered; also drifted below 75 pre-gate. `@musterd/mcp`: **≥ 75%**. The interactive onboarding wizard (`cli/src/onboard`) and the MCP tool handlers (`mcp/src/tools`) are covered by behavioral tests that drive `runInit` with `@clack/prompts` mocked and call each tool handler against a stub client (mcp went 57→64 with the OTel-propagation tests, then to 75 with the tool-handler tests).
