...
MUSTERD_DRIVER     = nick          # optional MANUAL OVERRIDE — NOT written by provisioning (ADR 165 inc 2: `musterd agent --driver <you>` writes binding.json's `driver`, per-worktree); the human driving this session (driver co-presence, ADR 021/155); the roster then shows 'driven by nick' AND reads the steering human working/present
...
**Cross-worktree binary guard (ADR 213 / 218).** ADR 143 refuses a foreign `MUSTERD_BINDING` when the cwd
...
stderr at config load and does not refuse to boot. Both roots must be seat workspaces that look like
real git checkouts (`.git` present); ephemeral fixtures (tmpdir + test `binding.json`, no `.git`) stay
silent (ADR 218). Packaged installs (no seat binding above the module path) stay silent. Fix by
pointing the harness MCP entry at this workspace's own dist (or a packaged `@musterd/mcp`) and
reloading.
...
`structuredContent` travels without an `outputSchema` (verified against SDK 1.29 and 1.30) — deliberately unregistered, because output schemas would ride `tools/list` and pay per-seat connect tokens the inc-2 pass just reclaimed. (MCP spec RC `2026-07-28` loosens output schemas to full JSON Schema 2020-12; that doesn't change the token calculus, so any adoption is measure-first — ADR 175's deferred list.)
...
src/
  ...
  binding.ts      // locate + parse .musterd/binding.json + the committed .musterd/workspace.json (ADR 018/080; shared format with the CLI); clearGrantFromBinding on stale-grant refuse (ADR 193); ADR 143 env leak guard + ADR 213/218 foreign-binary warn (real checkouts only); saveBinding preserves omitted capture fields and claimed seat_credential, capture writers pass { drop: { model_observed: true } } (ADR 270/340)
  ...
  tools/
    ...
    teamMemory.ts // team_insight_save + team_insight_search (alias team_memory_search) — findings for the whole team, pull-only retrieval (ADR 327)
