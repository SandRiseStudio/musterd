...
src/
  ...
  landed.ts       // lane 01M2XAXRP3: reconcile the brief's carried lanes against origin/main with this worktree's git before `team_next` renders them — a landing DECLARES its lane (`Lane \`id\`` in the body's first lines, `(lane id)` in the subject) or the lane's pushed branch is gone from origin; absent evidence is silence, never "not landed"
