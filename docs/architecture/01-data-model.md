...
- **v15 (`musterd/0.3`, ADR 101):** `ALTER TABLE presence ADD COLUMN model TEXT` + `ALTER TABLE requests ADD COLUMN model TEXT`. Model attestation: the harness-attested model id rides the **occupancy** (the `presence` row), never the durable seat — a different harness can occupy the same chair tomorrow with a different model (ADR 087). Attested, never verified (only the harness knows); nullable and rendered `unknown` when absent (warn-never-block). Captured at claim, re-attested on heartbeat, kept **sticky** across ambient touches (`COALESCE(?, model)`). `requests.model` carries a grant-less claimant's attestation across the admin-approval gap so the approved occupancy isn't born `unknown`. The switch history lives in the audit log (`occupancy.model_attested`, source `claim|heartbeat`), not a column. Both additive ALTERs; pre-v15 rows read `null`. The per-act stamp (`meta.model`, server-controlled) is derived from this occupancy value at send time. See [101](../decisions/101-model-as-a-variable.md).
