...
src/
  ...
  guidanceAttestation.ts // the guidance epoch this Workspace is RUNNING, read from its installed files rather than from the build's ceiling; cwd-defaulting wrapper over protocol's installedGuidanceEpoch so the CLI and the MCP adapter cannot disagree about one folder, and never memoised because a refresh rewrites those files mid-session (ADR 417)
  test-auth.ts        // CLI integration fixtures: claim helper (bootstrap key → agent-seat credential + Presence-bound lease, ADR 337); bindTempWorkspace — a folder bound to a vault member, how a fixture acts as a second member now --as is gone (ADR 442)
  ...
  trace/              // ADR 445 — the trace tap, client half: R1 hooks + R2 transcript tail
    ...
    transcript.ts     // R2 parsers (increment 2), version-stamped (`claude-code@1`, `codex@1`) and tolerant: Claude groups one API message's split jsonl lines by message.id (one usage per message, joined by its first tool_use id); Codex binds a reasoning item to the NEXT call_id and reads token_usage_record (token_count is a recognised skip — same counters, would double); an unrecognised record is `{kind:'unknown', bytes}` under UNKNOWN_RECORD_CAP, never dropped silently — the on-disk format is NOT a contract
    tail.ts           // R2 orchestration: per-session offset cursor in .musterd/trace-tail.json (TAIL_SESSION_CAP, path-pinned), readTranscriptDelta (whole lines only, partial trailing line held back, TAIL_MAX_DELTA_BYTES per run), buildTailEvent (content only past the 1b gates, scrub-then-cut), chunked posts that advance the cursor only when every chunk landed; a truncated file or a parser throw downgrades the session (cursor flag + one `unknown {downgraded}` event → the daemon's trace.downgraded audit row). Runs inside the hook processes that already exist (Claude Stop / SessionEnd capture, Codex post-tool-use / end) — no long-lived tailer
  ...
  broadcast/          // the hosted half of `musterd broadcast` — run the capture on a rented machine
    ...
    streamState.ts    // ADR 293 desired-state file (start/stop provenance: who, when, why) + the pure ensure decision — a machine gone while this says live is a crash by definition; 3 restarts/30min then stand down; recorded broadcast args are replayed on relaunch and named in the note
  ...
  host/               // the `musterd host` wake actuator — harness residency's per-machine hand (ADR 131 inc 3)
    ...
    wakeCapture.ts    // ADR 436 clause 4 — a wake that settled is over: endWakeCapture stamps `ended_at` on the capture a backend's settle CLAIMS (its ids; Grok's `wake-<lease>` placeholder renamed to the real headless session), applied by the loop at settle; never a session that took the slot mid-wake, never an already-ended one
    ...
    backends/
      ...
      grok.ts         // backend #5 (ADR 352 §7): `grok -p` fresh / `grok -r <id> -p` resume; no --yolo; PATH-pinned musterd; deferral taxonomy matches siblings; ADR 436 clause 1: settles with duration + the settle line and reads Grok's usage.json at exit (never resolves undefined)
      grokUsage.ts    // ADR 436 clause 2: the Grok cost reader — parseGrokUsage (pure, `session.*Tokens` → WakeUsage) + readGrokWakeUsage (newest usage.json under `$GROK_HOME/sessions/<encoded cwd>/` at-or-after the spawn); costUsdTicks is never converted — the row says harness_price_unverified
  ...
  session/            // session capture (ADR 131 §5, inc 4) — the machine-local judgement layer
    attendedProcess.ts // ADR 444 host backstop: attendedHarnessProcess(workspace, harness) — an open, non-headless harness process (claude without -p/--print) whose cwd is the worktree or under it, read from `ps` + the process cwd (lsof on macOS, /proc on Linux); a wake child is headless and never counts; undefined = "cannot tell" (unknown harness or unreadable table), never "none"
    ...
    transcript-model.ts // readModelFromTranscript(path) + resumeWeightBytes(path) (ADR 427: the user+assistant bytes a resume replays): the ONLY module that knows a harness transcript's on-disk shape — newest assistant turn's model, bounded tail, never throws (ADR 158)
  ...
  onboard/            // the `musterd init` interactive onboarding (@clack/prompts; ADR 005)
    ...
    workspace.ts      // provisionWorkspace(name): an isolated member Workspace (ADR 065) — in a repo, a git worktree at `~/musterd/<team>/<repo>/<name>` via memberWorkspaceDir (ADR 447; a pre-layout sibling `<checkout>-<name>` is reused; `branch` / `gitIdentity:false` for a human's worktree); outside git, a sibling folder
    ...
    driftCache.ts     // `.musterd/drift.json` (DriftCacheSchema): the provisioning-drift cache the MCP adapter reads (ADR 408 inc 4). refreshDriftCache re-inspects when absent, past DRIFT_CACHE_TTL_MS (10m), or written against a DIFFERENT daemon build; an UNKNOWN build never invalidates, or the interrupt probe would re-inspect at every tool boundary. `force` re-inspects regardless, and every caller that just repaired the folder passes it — the session-start heal and `init --refresh-{hooks,guidance,permissions}` — so a repair is never reported as still owed for the rest of the TTL. Counts, never paths. A LEAF by design — inspection and tombstone read are injected, so it stays out of the doctor↔selfHeal cycle; the production wiring is `refreshWorkspaceDrift` in doctor.ts. Never creates `.musterd` in a folder that has none; a clean folder still gets a zeroed row, because absent and clean are different states
  ...
  commands/
    ...
    session.ts        // musterd session start|end|observe --stdin (hook-driven capture / Cursor model observe, ADR 198/265/268) | observe --orient (ADR 333: Cursor sessionStart JSON additional_context) | resolve-labels --stdin (sidebar sweep decision engine) | label-nudge (a silent no-op since ADR 442 retired the peer sweep; kept so pre-wall hooks stay quiet) | orient-nudge / orient-stamp (per-session orientation ritual, ADR 326: stamp keyed to the captured session id quiets the per-turn nudge) | show (ADR 131 §5 / ADR 160 / ADR 186). `session start` and Cursor `observe --orient` run the ADR 419 self-heal probe before emitting orientation. `session start` also emits the ADR 326 orientation block after capture (read-only, wake-suppressed, composable-only). Interloper gate: an empty newcomer cannot take a live-looking slot; a named-but-missing occupant transcript is live by construction via started_at for LOCAL_SESSION_LIVE_MS (file appears at first turn, not at start). ADR 268: a new Cursor conversation_id without a model drops leftover model_observed; observe with no session_id still reconciles from enumeration; refresh heals an unended Cursor slot whose live .txt disagrees; a quiet unended slot contradicted by a live session is healed onto the newest one and the model is read from there (ADR 455)
    ...
    trace.ts          // musterd trace hook --stdin [--harness <id>] [--kind <event>] — the standalone ADR 445 tap on the Claude Code events no other musterd hook rides (UserPromptSubmit, PostToolUseFailure, Stop, SubagentStart/Stop, PreCompact); PreToolUse/PostToolUse/SessionStart/SessionEnd tap from inside gate / inbox --interrupt-check / session start|end so a tool call spawns no extra process; always exit 0
    gate.ts           // musterd gate check --stdin — the PreToolUse enforcement gate (hook-driven): match the tool call vs the team's class table client-side, adjudicate matches via POST /gate; fail-open (ADR 150). ADR 442: session-reach tools are refused locally in a seat Workspace before that round trip; SendMessage is allowed only toward a subagent this session spawned
    subagentLedger.ts // ADR 442: per-session spawned-subagent ids (sha256-16 filename); gate record-subagent --stdin reads the Agent tool_response
    host.ts           // musterd host [--once]: the resident wake-actuator loop (notify-shaped; ADR 131 inc 3)
    human.ts          // musterd human <name>: the mirror of `agent` — stands a person in a member worktree (~/musterd/<team>/<repo>/<name>, ADR 447; `--home <dir>` outside a repo) with their 0600 binding, mints/reuses/re-issues the credential, self-claims, sets current (ADR 176)
    ...
    service.ts        // musterd service install/uninstall/start/stop/restart/refresh/status/logs (ADR 045); refresh = sync main + build + restart in one guarded verb (ADR 118); the --auto tick also owns the SHARED Codex common-dir hooks (ADR 408 inc 5) — `withinWorktreeOnly` makes every seat skip that one file because it is one copy for the whole workspace family, so the refresher, which runs outside any seat and already owns the shared checkout, is the only thing that can ever repair it; refreshed AFTER the bounce verifies (a build that did not land would write this build's hook text for a daemon running the old one), never fails the tick, and the in-band bounce line says so only when something was written
...
Codex desktop is not a `musterd host` backend. Its acceptance evidence is the manual matrix in
[`tests/codex-desktop.md`](../../tests/codex-desktop.md): project MCP load, explicit join,
delivery/drain, reload/reconnect, isolation, and model/build attestation. Manual reopen and
`/mcp reload` remain manual resume, not daemon wake, until a versioned supported desktop API proves
exact task targeting, lifecycle observation, and safe resume (ADR 216).
...
path, else cwd walk-up **from the real path** — a symlink into a Workspace resolves as the folder it
really is, reach spec §6) → this global config**. The env + binding paths key identity to the
...
2. **Folder check** — first the hard refusal (reach spec §6, ADR 442): `bindingRefusal(cwd)` (`onboard/guard.ts`) exits 2 when cwd is `~`, `~/musterd`, a team root `~/musterd/<team>`, a repo group `~/musterd/<team>/<repo>` (ADR 447 — refused by position, even while empty), or has a Workspace (`.musterd/binding.json`) up to two levels beneath it — by real path, so a symlinked alias is refused like the real folder — because identity walks _up_ and a binding there would confer itself on every unbound folder under it. The member Workspace layout is `~/musterd/<team>/<repo>/<member>`; only that fourth level is a Workspace. `musterd human` refuses the same way; `musterd claim` only for a first binding. Then the soft guard (ADR 020) — before anything is written, `inspectInitTarget(cwd)` (`onboard/guard.ts`, pure + non-throwing) flags a wrong-looking target and, if any heuristic trips, shows the warnings and a single confirm defaulting to _yes_ (declining makes no changes, exits 0). Because the binding is per-folder, a wrong-folder run is a multi-artifact slip; this catches the common ones. Heuristics: (a) cwd **is the musterd source tree** (`package.json` named `musterd-monorepo`, or the `packages/{cli,server}` layout) — the 2026-06-15 dogfound slip; (b) cwd already has a **`.musterd/binding.json`** (read via the shared `BindingSchema`/`BINDING_*` constants) — init will mint a new member and _repoint_ the binding here; (c) cwd has an **`AGENTS.md` without the musterd primer markers** (`hasPrimerMarkers` from `primer.ts`) — the primer would append to it. **Warn, never block:** init stays runnable in any folder the user genuinely means (including this repo, for dogfooding), and a guard error never crashes init. A fourth check — _name already bound in another folder_ — needs the chosen name and so runs at step 6.
...
Either loop still also requires the target seat's residency `flow: auto`; the command only controls
the Team-wide half of the gate. A stakes default only applies at `lane_open`, and only when every
declared surface sits under the rule — mixed web+server lanes stay `normal`.
...
- With `--wait [--timeout <s>]` (ADR 054): a **blocking one-shot** over the same watch socket — drains the durable inbox first (startup-race guard), then blocks until the next **directed** act for this seat arrives, prints it, and exits **0** (or **124** on timeout; default bound 300s, `--timeout 0` unbounded). `--from`/`--act` narrow what wakes it; `--json` prints the raw envelope. The no-poll wake-on-message primitive — pairs with `/loop`: `musterd inbox --wait && <work>`. It marks the woken act read (unless `--peek`) so the same act can't immediately re-wake the next loop.
- `defer <act_id> --until-lane <lane_id> | --until-reply` (ADR 211): postpone one directed act until a condition fires. Sends a **deferring `wait`** (`meta.defer_ref` + `meta.until`) — exactly one condition; the two are different questions ("blocked on that work" vs "waiting on that person") and accepting both would leave which one fires ambiguous. **There is no time form**: the daemon runs no clocks (ADR 179), so "later" is a state edge. The deferred act stays unread and comes back on its own when its condition fires — even if the read cursor has passed it. It also stops being a wake reason while deferred. Deferring is legible: it is an act on the thread, so the sender can see the postponement rather than watching their ask rot unread.
- `--deferred` lists what you have postponed, marking each `● raised` or `○ waiting`. A **lens**, like `--from`/`--act`: it never advances the read cursor. The default view carries a `N deferred, M raised` footer — deferred acts are demoted, never hidden, so the unread count stays honest (ADR 117).
...
The **headless, no-prompt counterpart to `init`** for a folder carrying a committed `.musterd/workspace.json` (the secret-free launch spec, ADR 080). Reads that spec (`server`/`team`/`surface`/`claim`), resolves the team agent key from **local** sources only (`--key` → `MUSTERD_AGENT_KEY` → the machine's global `config.agentKeys[team]` — never the committed file, which has no secret), and registers the musterd MCP server for this folder (idempotent — `configure` does `mcp remove` then `mcp add`). It configures **the harness the folder declares** (`spec.surface` → the ADR 038 registry; a surface no adapter answers to, such as `cli`, falls back to Claude Code) — so a Codex or Cursor folder gets its own entry rewritten, and never a first install for a harness it never picked. That is also what makes `wire` a real repair for a baked-env entry in `.codex/config.toml` / `.cursor/mcp.json`, which the doctor prescribes (ADR 168: a detector whose fix has no safe form is half a feature). It also writes the gitignored `.musterd/binding.json` (spec + resolved secrets) so subsequent CLI acts resolve identity. **Registers tools only** by default (no autojoin) so a repo cloned by many never has every clone auto-claim one seat; `--autojoin` opts a personal worktree into claim-on-launch via `binding.autojoin` (ADR 165 inc 2), and a re-wire carries an existing `autojoin`/`driver` forward like `model`. Keyless → registers anyway (tools available) and warns that claiming will need a key/approval. This is what lets a fresh clone self-wire without an interactive `init` (the ADR-060 non-goal, unblocked). Output: `✓ wired the musterd MCP server for this folder (team <team>)`; `--json` → `{ team, member, mcpRegistered, keyResolved, autojoin }`. Errors: no committed spec → `not_found` (exit 6).

With `--migrate-bootstrap` (ADR 350/440), `wire` exchanges the binding's legacy Team key plus its
independent agent-seat credential for a seat-scoped successor, then atomically replaces only
`agent_key` in the protected binding. The server selects a 90-day expiry and returns its redacted
`expires_at` metadata; the plaintext is written directly and never printed. A publication failure
leaves the legacy binding intact. Rerunning replaces only the unused successor and starts a fresh
90-day window; a successor that has authenticated is never replaced by this path.
