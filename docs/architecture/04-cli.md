...
**`--live` (ADR 124):** the same verbs, retargeted at the **`/live` web viewer** instead of the daemon, so it's a managed service too — not a pile of hand-authored scripts. The viewer is a **two-agent bundle** in the same user domain: `studio.sandrise.musterd-live` (KeepAlive dev server on `:5173`) + `studio.sandrise.musterd-live-sync` (a `StartInterval` main-tracker). `install --live` **generates** two shell scripts (`~/.musterd/live/{serve.sh,sync.sh}`, from versioned builders in `launchd.ts` — reproducible + testable, not per-machine) and their two plists, ensures a **dedicated detached-on-`origin/main` worktree** (`…/agents-live`, added from the daemon's checkout — they share the git object store; never the `main` branch → no worktree contention), then bootstraps both. `serve.sh` checks out the tip of main → builds the one workspace dep the web app imports as dist (`@musterd/protocol`) → `exec pnpm dev`; `sync.sh` polls `origin/main` and `kickstart`s the server when it moves, so **`/live` tracks main with zero manual step** (a full restart, not an in-place checkout, so the open tab reliably reloads and any change type — web/protocol/deps — is picked up). `refresh --live` forces that sync now; `status --live` reports both agents + probes whether `:5173` is actually serving; `logs --live` tails `viewer.log`+`sync.log`; `uninstall --live` boots out both + removes the generated artifacts (leaves the worktree unless `--purge`). Unlike the daemon verbs, `--live` ops **skip the shared-daemon guard** — the viewer is a dev-side web tab and drops no teammate session. Like the daemon, it's macOS-only via the same seam. `renderPlist` in `launchd.ts` is the one XML template both the daemon and the two viewer agents share.
