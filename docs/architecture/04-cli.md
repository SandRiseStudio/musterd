...
src/
  ...
  host/               // the `musterd host` wake actuator — harness residency's per-machine hand (ADR 131 inc 3)
    ...
    backends/
      claudeCode.ts   // backend #1: explicit portable/fresh orders bypass resume; legacy orders retain the resume ladder (`--resume <captured id>`, 30d GC + transcript-hygiene rungs) degrading to fresh `claude -p` in the same lease; reports actual delivery + local byte/age metadata only (ADR 131 §5 / ADR 209) Under ADR 210 an order marked resume_eligible takes the exact-match rung INSTEAD of the slot/enumerated ladder: registry hit + byte/age hygiene or fresh; a non-eligible order never reads the registry at all.
      codex.ts        // backend #2: exact JSONL thread identity; `exec resume` in the workspace cwd then one fresh `exec -C` fallback, each requiring wake-provenance Presence; sanitized environment and no bypass flags (ADR 216)
