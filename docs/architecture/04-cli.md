...
src/
  ...
  commands/
    ...
    session.ts        // musterd session start|end --stdin (hook-driven capture) | resolve-labels --stdin (sidebar sweep decision engine) | label-nudge (evidence-based due, single CCD scan, ADR 186) | show (ADR 131 §5 / ADR 160 / ADR 186)
