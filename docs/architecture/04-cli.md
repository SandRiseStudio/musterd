...
src/
  ...
  onboard/            // the `musterd init` interactive onboarding (@clack/prompts; ADR 005)
    ...
    workspace.ts      // provisionWorkspace(name): an isolated seat Workspace (ADR 065) — in a repo, a git worktree at `~/musterd/<repo>/<name>` via memberWorkspaceDir (reach spec §6 / ADR 442; a pre-layout sibling `<checkout>-<name>` is reused); outside git, a sibling folder
