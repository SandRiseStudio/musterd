...
src/
  ...
  transport/
    ...
    oauth.ts          // OAuth 2.1 authorization server (ADR 446 §3): discovery metadata, DCR (public clients, validated redirects), consent page, code/token/revoke; TLS-except-loopback, per-IP buckets, `oauth.*` audit, OAuth-shaped token errors
