...
src/
  ...
  store/
    ...
    review.ts         // the ADR 169 review-counterpart picker: pickReviewCounterpart (risk-tagged → live human/admin first, else a live seat whose model FAMILY differs from the worker's per ADR 158 attestation; unknown family ineligible; nobody → null = sanctioned self-close) + memberFamily/workerFamily + teamFamilyPosture (ADR 172/187/189: idle wake_pool carries family + wakeability mark from residency — mark-not-filter)
