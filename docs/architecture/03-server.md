...
src/
  ...
  store/
    ...
    trace.ts          // ADR 445 R1 (inc 1a): ingestTraceEvents (structural columns only — no parameter for content; daemon-assigned monotonic seq per (team, session), one transaction per batch) + listSessionTrace + countTraceEvents, all against the TRACE store
