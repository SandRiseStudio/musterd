...
src/
  ...
  store/
    ...
    goals.ts          // declared-Goal seam: listGoals (meta.goal messages, status + epoch derived) + nextGoal + goalEpochBumps (ADR 048/084/109)
    staleness.ts      // stale-plan detection: stale_plan/stale_dependency lane warnings from goal-epoch vs claim-time (ADR 109)
