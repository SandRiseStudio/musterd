...
src/
  ...
  db/
    ...
    traceDb.ts        // ADR 445 §3: openTraceDb(path) + TRACE_MIGRATIONS — `trace.db` beside `musterd.db`, its own schema_meta and ladder (`migrations:check` parses both); traceSchemaVersion for /health
...
- **`GET /teams/:slug/trace/sessions/:digest`** (increment 2, behind `musterd trace show`) — `200 { events }`, seq-ordered, `detail`/`content` parsed back to objects. Ordinary authenticated read (`authTouch`), scoped per ADR 128: a seat gets its own rows; an `is_admin` seat gets any seat's; everything else filters to empty — "not yours" and "not there" are deliberately the same answer, so the route is no oracle for which digests exist.
