...
src/
  ...
  transport/
    ...
    mcpHttp.ts        // Streamable-HTTP `/mcp/:team` (ADR 446 §1): SDK `createMcpHandler` framing with a per-request seat-bound factory; Done-line tools (`team_join` no-op, `team_inbox_check`, `team_send`) over the REST routes' own store paths; ambient-presence stamp per call
