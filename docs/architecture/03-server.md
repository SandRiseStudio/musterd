...
src/
  ...
  protocol/
    ...
    laneReroute.ts    // re-routing an acceptance away from a reviewer who is no longer there — one implementation, two triggers: rerouteDepartedAcceptances (ADR 412, the explicit `/residency/session` goodbye) and sweepReapedAcceptances (ADR 413, the reaper noticing a silent departure). Owns the acceptance-ask composition (acceptanceAskBody/priorOwnerNotice/noGoalNotice) and delivery (deliverLaneAskAct/deliverLaneAskSuperseded) because the reaper needs them and `presence/` must not reach up into `transport/`
