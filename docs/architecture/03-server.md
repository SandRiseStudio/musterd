...
src/
  ...
  store/
    ...
    review.ts         // the ADR 169 review-counterpart picker: pickReviewCounterpart (risk-tagged → live human/admin first, else a live seat whose model FAMILY differs from the worker's per ADR 158 attestation; unknown family ineligible; nobody → null = sanctioned self-close) + pickWakeReviewer / reviewLoopBounceCount (ADR 191: offline wakeable pick + circuit breaker) + memberFamily/workerFamily + teamFamilyPosture (ADR 172/187/189: idle wake_pool carries family + wakeability mark from residency — mark-not-filter)
    residency.ts      // the wake ledger: residency enrollment + wake leases — claimWakeLeases (transactional derivation: immediate/batched inbox + ADR 191 work_order review asks when loops.review ∧ flow:auto, defer-snoozed) / settleWakeLease / expireWakeLeases / recordSessionAttestation (harness-class-only, inc 4); rate policy derived from residency.* audit rows (ADR 131)
...
- `listPresence`/roster `summarize` resolve a coarse `activity` per Member by the **two-clocks rule** (`store/activity.ts` `resolveActivity`): the liveness clock (fresh presence?) decides `offline` vs present; the status clock (latest `status_update`, via `latestStatusUpdate` — prefers `meta.state`, falls back to body) decides `idle` (ADR 140; was `online`) vs `working`. A third signal, **steering** (ADR 155 Inc 1), overrides both: a human named as the `driver` of a *live* agent seat (`listLiveDrivers`) reads `working` and present without their own heartbeat — derived here at read time, no synthetic presence row — so a driving human stops reading offline on the roster (the driver-copresence fix). The backing summary is returned as `state`, with `last_status_at` driving the CLI's `· <age>` staleness suffix. These are **additive** roster fields; a v0.1 reader ignoring them still conforms.
