...
src/
  ...
  protocol/
    ...
    nudge.ts          // the ccd delivery rail's daemon half (ADR 167): composeNudgeLine (deterministic, structured-fields-only — the ADR 128 discipline) + deliveryHintFor (predicate: directed hint-act, live recipient, derived 10-min damping) + confirmNudge (recompose-and-compare relay confirmation — nudge/verbatim, no pending-nudge store)
  notify/
