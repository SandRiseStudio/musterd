...
src/
  ...
  sync/
    ...
    fold.ts           // the fold (ADR 325 inc 3b-ii): foldBatch / readPullCursor — the ONE foreign-origin writer into `messages`, run by hub and puller alike. Per event in hub_seq order inside one transaction: skip own origin, skip a held (origin_node, origin_seq) pair (the idempotence key, v52 unique index — never messages.id), refuse a read-side origin gap, resolve from AND to by seat NAME or block the cursor AT that event with the prefix committed, refuse an act this build's ActSchema does not know, refuse an id held under another origin. Never reads or writes nodes.next_seq (fold.test.ts's first case is the falsifier); created_at is THIS daemon's clock, never envelope.ts
