...
- **What is captured today (Claude Code):** `SessionStart`/`SessionEnd` from the capture hook, `PreToolUse` from the gate (on the tools it is registered for), `PostToolUse` from the interrupt probe (every tool), and `UserPromptSubmit`/`PostToolUseFailure`/`Stop`/`SubagentStart`/`SubagentStop`/`PreCompact` from the standalone `musterd trace hook`. Codex, Cursor and Grok tap nothing yet (the 1a tail); rail R2 (transcript tail) and the content column are increments 2 and 1b.
