...
- **Audit sizes-only.** `memory.save`/`memory.clear` audit actions carry `size_bytes`/`headline_len` in `detail`, never the headline or body text (hard rule 5). The HTTP surface (seat-authenticated `PUT`/`GET`/`DELETE /teams/:slug/memory`) and the envelope-on-occupy wiring are the delivery layer (Task 4).
