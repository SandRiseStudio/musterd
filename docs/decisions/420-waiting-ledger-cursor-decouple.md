...
- **Reconciliation with ADR 088 Amendment 3 (added on review decline, lane `01M2TRW6R8`).** Decision item 3 says no surface treats cursor passage as discharge ever again; Amendment 3 says rendering IS discharge — for acts with no answering move (steer, urgent message), via an explicit `inbox.rendered` audit row per (recipient, act), because the ADR 287 watermark cannot carry them. Both stand: 420 forbids the READ CURSOR (watermark) from discharging anything; 088 (iv) discharges through a separate recorded row, and `readAlready` (`packages/server/src/transport/http.ts`) is its carrier in the inbox reply. Nothing on main stated this, which is how a future reader deletes `readAlready` in good faith believing 420 forbids it — this note is the thing that stops that deletion.
...
- **Eval:** the falsifier is ryder's episode, runnable by hand — on a seat whose owed set is non-empty and holds an act older than the newest unread, render the full inbox answering nothing, then re-read the banner. Fixed means the count and floor do not move; the current code moves both. Two preconditions are load-bearing, and the second was caught in review (ryder, decline on lane `01M2TRW6R8`): (a) the owed set must be non-empty — on an empty set the test passes vacuously, including on unfixed code; (b) the held act must be one WITH an answering move (ask / request_help / handoff) — doorbell clause 7(iv) / ADR 088 Amendment 3 discharges a steer or urgent act on being rendered, so a held steer moves the count on fixed code for a sanctioned reason and the test would report broken. Baseline: 63 → 60 on one render, floor +12h.
