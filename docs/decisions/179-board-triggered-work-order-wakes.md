...
| Seat drop B — restarted adapter reads unclaimed forever                              | `01KYQCF678TC29ARGWRFD2H4A5` | **done** — ADR 193: expired grant is `refused`/`expired_grant`; client drops it + retries bare; pending-presence guard surfaces `lastJoinError`                                                                 |
...
| Roster hygiene — stale departed-seat claims, ~45 junk `web-*` members                | `01KYQ91AWP66ZSQDYBB2M3WTKP` | **done** — ADR 196: `leaveMember` + reaper release in-flight claims; idle observer cap (8/team) additive to ADR 064 TTL                                                                 |
