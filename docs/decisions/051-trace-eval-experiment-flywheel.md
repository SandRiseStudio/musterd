...
_coordination_ lens is the standalone product (working name **batond**, ADR-reversible). This ADR
...
_between_ agents and humans (MAST: ~79% of multi-agent failures live there).
...
- **Emit in musterd, engine in batond.** musterd's job is to _emit the best coordination trace in
  existence_ (Layer 1 + Layer 2, observability.md §4–5). The **eval + experiment engine is batond's
...
- **The moat is coordination-native, on both ends:**
  - **Trace:** the unit is the _team task_, not one agent's chain. Spans include agent-turn detail
  ...
  - **Eval:** the unit is the _team outcome_ (did the human+agent team hit the Goal's definition-of-done),
    ...
    a Goal's DoD _is_ an outcome-eval target; derived status _is_ a programmatic eval signal. Evals are
...
- **Prompts in traces are opt-in and versioned.** Capture the prompt as a versioned artifact (hash +
  ...
  **opt-in with redaction + retention policy** (extends observability.md §4's "never the body" stance and
  ...
  text is gated.
  > **Split out 2026-07-30 → [ADR 184](184-dataset-consent-and-redaction.md).** These two sentences were
  > load-bearing for something they did not cover: `docs/research/README.md` gated the whole dataset on
  > "the opt-in + redaction posture (ADR 051) being enforced", but this bullet governs **prompt text in
  > spans**, while the dataset is built from the **message log** — a different body of data, with the
  > only surface in the system that stores prose verbatim. Combined with this ADR sitting at `proposed`,
  > the gate could never open. ADR 184 owns consent + redaction **for published artifacts**, is
  > decidable without resolving this ADR, and carries the gate's definition of done. The direction above
  > stands as strategy for trace emission; it is no longer the dataset's gate. This is the
  > "each gets its own ADR when built" line in Consequences, honoured.
...
- **Harness-decay thesis (the contrarian wedge).** As models improve, scaffolding yields diminishing
  ...
  _measure that decay_, telling us when to **delete** musterd/harness complexity. A coordination tool
