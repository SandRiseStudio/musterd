...
- Builds on: [ADR 039](039-cross-network-topology.md) (one Team, one authority; overlay-first
  ...
  released remote MCP/OAuth route and event-membership flow; otherwise do not expose the hostname.

## Context

The demo goal needs a remote MCP app on a phone to reach one Team on the presenter's laptop. A private overlay is the right default for teams whose Members can join it ([ADR 039](039-cross-network-topology.md)); an audience at an event cannot be asked to install or join one. A public hostname is therefore needed for the phone-app flow.
...
- A demo has a distinct daemon authority and disposable state. No protocol schema, runtime dependency, or shared-daemon configuration changes.
- TLS terminates at Cloudflare; the connector-to-daemon hop is HTTP over loopback inside the presenter's host. The proxy-trust flag is safe only while that origin remains loopback-only and the host remains trusted.
...
- The named demo Tunnel's DNS route and scoped credential outlive a stopped connector unless explicitly removed. Teardown removes both and invalidates residual Team/OAuth credentials by revoking Members and deleting the isolated database.
- Follows-up: `01M3AKN6GFHZ3DGCJPRE4RG2TE` — remote MCP/OAuth implementation and its release security gate.
