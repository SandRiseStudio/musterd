...
| `POST`   | `/teams/:slug/claim`                           | stateless claim mirror of WS `claim` — carries `workspace` + `workspace_key` (ADR 014/368), so a stateless claim is scoped by workspace exactly as a WS one is        |
