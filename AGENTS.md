...
- **`ROADMAP.md`** — what's next (reserved-but-unbuilt, out-of-scope-by-principle). Forward-looking; replaced, not accreted. **Its item list is generated** from `content/roadmap.data.ts` — the single source of truth — via `pnpm roadmap:gen` (ADR 041). This file is the only rendering of it: the `/roadmap` page was dropped from the web UI on 2026-07-28 and `musterd.io/roadmap` 404s (corrected here 2026-09-21, lane 01M32QTVN). Edit the typed data module and regenerate; never hand-edit between the `<!-- GENERATED ROADMAP -->` markers (`format:check` runs `roadmap:check` to block drift).
...
**Your channel.** Prefer the `team_*` tools (the musterd MCP server) when this session has them; the
`musterd` CLI resolves the SAME seat and is right where no tool exists (`session orient-stamp`).
Run it **from this Workspace** — else it acts as that folder's seat. `musterd whoami` says who.
