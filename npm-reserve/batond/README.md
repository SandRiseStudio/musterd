...
Existing agent-observability tools see *inside* each agent (its LLM calls, tools, reasoning). batond sees the space *between* agents and humans: handoffs, help requests, blocking waits, ignored messages — the coordination failures that account for most multi-agent breakdowns. It ingests [musterd](https://github.com/) coordination logs natively and plain OpenTelemetry GenAI/agent spans, so it works whether or not you run musterd.
