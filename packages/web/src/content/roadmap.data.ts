...
const RAW: RawItem[] = [
  ...
  },
  {
    ...
    detail:
      'A living, human-vs-agent-neutral office view of the team (ADR 079 + office-rive-character-spec.md). ADR 079 shipped M1–M3: M1 (code-drawn isometric floor + act cues + panel modes), M2 (per-member Rive characters from public/office/character.riv driven by the officeToRig contract, plus acts as walking choreography — walk-over, carry-box handoff, megaphone broadcast), M3 (presence changes walk in/out, door-open staging, urgent walks at faster cadence, reduced-motion parity). The office renders live and degrades to the code-drawn avatar if the Rive WASM/asset fails. Then ADR 086 (ambient office life) added the calm-at-rest layer: Phase 1 GPU-composited ambient overlay + afterglow (idle-park invariant intact, rAF 0/sec at rest), Phase 2 idle micro-choreography (coffee strolls + idle-FPS cap + real-act preemption), Phase 3 render optimisation (Rive idle sprite-cache), and the ambient in-place gesture poses (Phase 2 tail — the gesture-rigged character.riv export + scale-aware bob/sway landed in PR #137). Remaining: overflow/nook polish, perf passes, and richer authored .riv fidelity. Shares the firehose/observer substrate with the web dashboard. The office/stream act vocabulary has since grown to cover the full act set: lane lifecycle events render as a distinct work-moving class (ADR 102), the steering acts get their own choreography (ADR 107 — steer sweep + redirect, challenge question, defer board pulse), and a reclaimable seat shows a "reconnecting" hint on the roster (ADR 105). The act→tone/label/glyph/choreography/sound seam is documented in docs/architecture/08-web.md.',
    refs: [adr(79, 'ADR 079'), adr(86, 'ADR 086'), adr(102, 'ADR 102'), adr(107, 'ADR 107'), doc('docs/architecture/08-web.md', '08-web.md'), doc('docs/design/office-rive-character-spec.md', 'office-rive-character-spec.md')],
