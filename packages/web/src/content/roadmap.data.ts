...
export const ROADMAP: RoadmapItem[] = [
  ...
  // L1 is verified emitting (finding 002) — the data is already useful, L2 makes it first-class.
  {
    ...
    detail:
      'The seed of a standalone coordination-observability product, and the head of the depth wave. Pulled up 2026-07-04: Layer 1 is verified emitting live (finding 002 mined ~53 h from the local sink and caught the broadcast-journal anti-pattern by hand) — so the data is already useful; L2 is what turns "grep a text log" into first-class MAST-aware views (ignored request_help, circular handoffs, stalled threads, broadcast-only journaling) + the report surfaces. Frozen as a three-increment arc (ADR 089). Increment 1 SHIPPED 2026-07-05: the shared @musterd/telemetry bootstrap boots in the MCP adapter (a musterd.tool.call span around every tool) and the CLI (musterd.cli.command; serve + interrupt-check carved out), so the ADR 011 meta.otel plumbing fires in production and a handoff is one cross-agent distributed trace — identity attribution fixed first (issue #107: normalized seat id as the key, raw name a label). Increment 2 (ADR 090, design frozen 2026-07-06): per-recipient delivery status derived from the log + cursors + audit — never a delivery table (logged / seen / answered per recipient, attempt history as telemetry, a seen_latency metric completing the ADR 088 raised→read pair, ledger surfaces on report) — the band.ai borrow (landscape.md §5) recast for seats. Increment 3: the MAST-aware views + report surfaces, on the ADR 050 projection seam.',
    refs: [
      adr(89, 'ADR 089'),
