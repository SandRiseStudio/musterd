...
/**
 * The office cat — behaviour + pose state only, no renderer (the painter lives in render.ts, the same
 ...
 *
 * The cat is deliberately shaped around the office's rest model (ADR 086): a *sleeping* cat is a static
 * pose, so the baked still frame holds it for free; the cat only asks for animation frames while it is
 * actually stretching/walking/sitting — `stepCat` returns whether it still needs the loop. Gait phase
 * advances from DISTANCE travelled, never wall time (the same no-skate rule the members follow).
 */

export type CatMode = 'sleep' | 'stretch' | 'walk' | 'sit' | 'curl';
...
export interface CatState {
  ...
  phase: number;
  /** Screen-space facing: true = the cat points left. */
  flip: boolean;
...
/** How far into a window's light pool the cat settles (matches the beam's bright end, not its faint tail). */
 ...
 * against the nav grid so the cat never beds down inside furniture.
 */
export function napSpots(daylight: number): CatSpot[] {
...
/** A fresh cat, asleep on its favourite rug (or the first walkable spot the room offers). */
export function createCat(rng: () => number = Math.random): CatState {
...
export interface CatBeatOpts {
  /** Current natural-light level (see lighting.ts) — steers day naps into the sunbeams. */
  daylight: number;
  /** Floor spots beside members currently working — the cat sometimes sits with them, supervising. */
  workSpots?: P[];
  rng?: () => number;
}

/**
 * Stir the cat (called on the office's ambient-beat timer): wake, pick a destination — usually the best
 * nap spot for the hour, sometimes a working member's side — and set off via the nav grid. No-ops unless
 * the cat is settled (asleep or mid-sit); returns whether a move actually started, so the caller knows
 * to keep the frame loop alive.
 */
export function catBeat(cat: CatState, opts: CatBeatOpts): boolean {
  if (cat.mode !== 'sleep' && cat.mode !== 'sit') return false;
  const rng = opts.rng ?? Math.random;

  let target: P | null = null;
  let plan: CatState['plan'] = 'nap';
  // Filter to spots the nav grid says are open floor — findPath tolerates blocked endpoints (it steps
  // out to free ground), but a cat *settling* inside a chair footprint would draw inside the chair.
  ...
  if (!target || Math.hypot(target.lx - cat.lx, target.ly - cat.ly) < MIN_TRIP) return false;

  cat.path = findPath({ lx: cat.lx, ly: cat.ly }, { lx: target.lx, ly: target.ly });
  cat.seg = 0;
  cat.plan = plan;
  cat.mode = 'stretch';
  cat.modeT = 0;
...
/**
 * Advance the cat by `dt` seconds. Returns whether the cat still needs animation frames — false only
 * when it is asleep, which is the office's cue that the room can park on a baked still frame again.
 */
export function stepCat(cat: CatState, dt: number): boolean {
  cat.modeT += dt;
  switch (cat.mode) {
    case 'sleep':
      return false;
    case 'stretch':
      if (cat.modeT >= STRETCH_S) {
        cat.mode = 'walk';
        cat.modeT = 0;
      }
      return true;
    case 'walk': {
      let travel = CAT_SPEED * dt;
      while (travel > 0 && cat.seg < cat.path.length - 1) {
        const next = cat.path[cat.seg + 1]!;
        const dx = next.lx - cat.lx;
        const dy = next.ly - cat.ly;
        ...
        cat.lx += (dx / d) * step;
        cat.ly += (dy / d) * step;
        cat.phase += step / STRIDE; // gait from distance, never wall time
        // Screen-space heading under the 2:1 iso: x grows with (lx − ly).
        const sx = dx - dy;
        if (Math.abs(sx) > 0.5) cat.flip = sx < 0;
        travel -= step;
        if (step >= d) cat.seg++;
      }
      if (cat.seg >= cat.path.length - 1) {
        cat.mode = cat.plan === 'sit-then-nap' ? 'sit' : 'curl';
        cat.modeT = 0;
      }
      return true;
    }
    case 'sit':
      if (cat.modeT >= cat.sitFor) {
        cat.mode = 'curl';
        cat.modeT = 0;
      }
      return true;
