import type { Activity } from './acts.wire.js';
 ...
 *
 * Validator-free on purpose: `posture.ts` builds the zod enum from these tuples, and the browser
 * reads them without pulling zod into its bundle (`guards.ts`).
 */
export const POSTURES = ['working', 'active', 'away', 'offline'] as const;
export type Posture = (typeof POSTURES)[number];

/** The wire-accepted posture spellings, including the legacy `idle`. */
export const POSTURES_ON_WIRE = [...POSTURES, 'idle'] as const;

/** Normalize an accepted wire spelling to its canonical {@link Posture}. */
export function normalizePosture(p: (typeof POSTURES_ON_WIRE)[number]): Posture {
  return p === 'idle' ? 'active' : p;
}
