 ...
 *
 * `idle` is the live-but-no-task state (activity `online`); `away` folds explicit `away`/`dnd`
 * availability. Offline *reasons* (signed off vs off-hours vs disconnect) are out of scope for v1.
 */
...
export interface PostureInput {
  ...
  availability?: { status: 'available' | 'away' | 'dnd' } | null;
}

/**
 * Resolve roster posture. Order matches the CLI status grouping (ADR 044):
 * offline → away/dnd → working → idle.
 */
export function resolvePosture(input: PostureInput): Posture {
  ...
  if (avail === 'away' || avail === 'dnd') return 'away';
