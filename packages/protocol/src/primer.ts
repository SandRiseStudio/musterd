...
function renderPrimer(identity: string): string {
  ...
  return [
    ...
    '',
    '**Your channel.** Prefer the `team_*` tools (the musterd MCP server) where this session has them; the',
    '`musterd` CLI resolves the SAME seat, so it is correct where no tool exists (`session orient-stamp`,',
    "`stream …`) — but run it **from this Workspace**, else it acts as that folder's seat. `musterd whoami` says who.",
    '',
