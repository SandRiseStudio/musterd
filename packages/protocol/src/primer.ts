...
export function renderPrimer(opts: {
  ...
  return [
    ...
    '- **Own your work in a lane.** `lane_open {title, surface_globs, claim:true}` / `musterd lane open',
    '  "<title>" --surface <globs> --claim` — declare what you are working on and the paths it touches',
    '  when you start a task, and `lane_resolve` / `musterd lane resolve <id>` when it lands. musterd',
    "  warns you (never blocks) if your lane overlaps a teammate's or builds on unfinished work;",
    '  `lane_board` / `musterd lanes` shows who owns what.',
