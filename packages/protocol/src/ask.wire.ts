/**
 * The to-human ask contract as plain TypeScript — no zod. `ask.ts` builds the enums from these
 * tuples and re-exports every name, so the browser can read species/tier and derive a clock without
 * pulling a validator into its bundle (`guards.ts`).
 */
