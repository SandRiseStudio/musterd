...
describe('db', () => {
  ...
  it('v66 rebuilds seeds so slack_user_id may be NULL, carrying every relay Seed over (ADR 373 inc 2)', () => {
    ...
    expect(runMigrations(db)).toBe(74);
  ...
  it('v18 backfills pre-existing observer seats to the full grade (no silent dashboard downgrade)', () => {
    ...
    expect(runMigrations(db)).toBe(74); // runs v18…v74 (governed authorization substrate, audit actor attestation, guidance epoch, doorbell, in_reply_to column, oauth, member sponsorship, agent connect nonces)
