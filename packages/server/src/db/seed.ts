...
export function seedDawn(db: Database): SeededDawn {
  ...
  const nick = addMember(db, team, { name: 'nick', kind: 'human', role: 'lead', lifecycle: 'forever' });
  const ada = addMember(db, team, { name: 'Ada', kind: 'agent', role: 'backend', lifecycle: 'session' });
  const lin = addMember(db, team, { name: 'Lin', kind: 'agent', role: 'frontend', lifecycle: 'session' });
  return {
    teamId: team.id,
