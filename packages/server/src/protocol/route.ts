...
function routeEnvelopeInner(
  ...
  /** ADR 241: which evidence answered — audited, never on the wire. */
  let handoffBasis: HandoffLaneBasis | undefined;
  if (env.act === 'handoff' && !(env.meta as { lane_handoff?: unknown } | null)?.lane_handoff) {
    // ADR 241: the recipient is evidence. A handoff act directed at a seat, moments after a lane
    ...
    if (derived.kind === 'attach') {
      ...
      // ADR 241: say which set was ambiguous. "you hold N" is false when the candidates are lanes
  ...
  if (handoffLane) {
    appendAudit(ctx.db, team.id, {
      ...
      detail: {
        message: message.id,
        ...handoffLane,
        // ADR 241: which of the two candidate sets answered. Always written when a derivation
        // happened, so absence means "recorded before ADR 241" and never "the held set" — the
