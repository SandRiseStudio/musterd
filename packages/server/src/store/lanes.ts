...
/**
 * Which evidence answered (ADR 241) — audited, never on the wire. `handed_to_recipient` is the
 ...
 *
 * ADR 241 CORRECTION. Held lanes are the WEAKER evidence and are now the fallback, because
...
/**
 * Lanes `sender` handed to `recipient` that `recipient` still holds and has not closed (ADR 241).
 *
