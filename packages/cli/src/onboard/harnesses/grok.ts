...
function upsertJsonHook(
  ...
  kept.push({
    ...(matcher ? { matcher } : {}),
    hooks: [{ type: 'command', command, ...(event === 'PreToolUse' ? { timeout: 10 } : {}) }],
  });
...
const LOCAL_HOOKS: readonly {
  ...
  },
  {
    ...
    command: cmd('inbox --interrupt-check', INTERRUPT_MARKER, true),
    missing:
      'the Grok PostToolUse interrupt hook is missing from .grok/hooks/musterd.json — a busy agent will not see urgent steering mid-loop (ADR 088). Run `musterd init --refresh-hooks`.',
  },
  {
    event: 'PreToolUse',
    marker: GATE_MARKER,
...
export function installMusterdGrokHooks(dir: string = process.cwd()): string[] {
  ...
  for (const spec of LOCAL_HOOKS) {
    ...
    const w = upsertJsonHook(path, spec.event, spec.marker, spec.command, spec.matcher);
