...
/* Chrome, its profile, its port and the CDP socket — `chrome.mjs`. That plumbing was written here
   one incident at a time and moved out on 2026-09-21 (lane 01M32HG5GJ) when the target-size sweep
   needed the same browser brought up the same safe way. The incidents it encodes are listed in its
   header; the one that matters most to THIS file is the port, because a sweep that reaches another
   sweep's browser files a green report about a page it never opened. */
const { send, exit } = await openChromePage({
  tool: 'contrast-sweep',
  verdictNoun: 'contrast',
});
