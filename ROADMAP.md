...
- **Service guardrails** · Platform — musterd service stop/restart warns when other members hold live sessions, so bouncing a shared daemon doesn’t silently drop a teammate. Ties the daemon lifecycle command to roster awareness. A dogfood finding — a shared daemon was restarted three times under a live teammate with no in-band heads-up. --force overrides. _Builds on Daemon service lifecycle._ ([ADR 047](https://github.com/SandRiseStudio/musterd/blob/main/docs/decisions))
...
## Current build sequence (reprioritized 2026-06-24)

The generated list above is grouped by category; this is the **priority order** we're building in.

- **Gate — ship v0.2.** Publish to npm + the launch post *before* new dev, so adoption feedback informs the rest. (On Nick: `npm login` + publish; everything else waits on this.)
- **Wave 1 — harden the coordination loop** (small, additive, no v0.3 dependency; the wedge's #1 priority, all evidence-backed by the 2026-06-24 dogfood): **Agent-side reachability** → **Service guardrails** → **Ambient agent presence** → **CLI ergonomics** → **Frictionless seat binding**.
- **Wave 2 — the v0.3 governance rock**: **v0.3 governance — seats, grants, capabilities** (the prerequisite), then the full governed **Notification tiers** it unlocks, then **Schedule & lifecycle enforcement**.
- **Wave 3 — reach + the second-product seed**: **Web dashboard** → **Work items, board & insight layer** → **Coordination-density insight**; **Telemetry — Layer 2 + SDK**; **iOS & Slack surfaces**.
- **Later / opportunistic** (no near-term pull): step-level streaming transport, team-to-team federation, role templates & mixed-harness teams, Python client SDK, sandboxed runtime.
