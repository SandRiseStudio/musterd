...
- **Telemetry & observability.** Near-term: minimal OpenTelemetry instrumentation of `@musterd/server` (one span per Envelope on the validate→persist→route path, act/team metrics; off by default, OTLP-out, no phone-home) plus the `meta.otel` trace-context convention (ADR 011) so a handoff links the sender's and receiver's traces across runtimes and vendors. Strategy, build/buy line, and the **coordination observability** product thesis (MAST-aware views over the act-typed log — the layer existing agent-observability tools can't see): `docs/design/observability.md`; standalone-product branding brainstorm: `docs/design/brand-coordination-observability.md`.
