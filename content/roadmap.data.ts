...
const RAW: RawItem[] = [
  ...
  },
  {
    ...
    category: 'harness',
    blurb:
      'A secret-free .musterd/workspace.json rides the repo, so a fresh clone/workspace registers the musterd MCP server with one no-prompt `musterd wire` — no interactive init.',
    detail:
  ...
  },
  {
    ...
    category: 'harness',
    blurb:
      'Codex CLI becomes a native, safe residency backend with the same coordination outcomes as Claude Code; Codex desktop is verified separately and stays manual-resume until a stable supported wake API exists.',
    detail:
    ...
    dependsOn: ['harness-residency', 'harness-adapters'],
  },
  ...
  },
  {
    ...
    shipped: { prs: [435, 439, 443, 444, 445, 446, 469, 470, 473] },
    category: 'human-loop',
    blurb:
    ...
    detail:
      'The record: nick created 5 lanes and owns 0 of ~84 ownerships; the one work item only he can do (publish the packages to npm) has sat parked and invisible for weeks — musterd literally cannot say "the team is blocked on nick’s lane." Nothing in the schema stops it (`owner_seat: nick` is already legal; lanes already have a backlog state); the only human claim surface is the CLI he never opens. So the board becomes writable from the web UI: a human creates a work item any time and picks it up, and all such work is captured/measured/auditable like agent work. No new hierarchy nouns — ADR 098 holds (Goal → Lane). First dogfood: a real publish-to-npm lane owned by nick, aging and nudgeable (release tooling now exists as `pnpm release` / ADR 156 — the remaining gate is credentials + running it). Builds on the read-only board (insight-dashboard increment 1) by adding write. SHIPPED 2026-07-27/28 by miley: Increment A made /board writable end-to-end — shared sign-in, the full lane lifecycle from the browser, live via refetch-on-lane-event (#435); Increment B added the insight rail over GET /report and a columns-versus-Goals swimlane toggle (#439); a polish pass added the per-member filter lens and visual depth (#443). ADR 170 then removed the last paste — `musterd board` opens /board already signed in, the CLI staging the credential it already holds behind a 60-second single-use nonce, redeemed once and stripped (#444/#445/#446). Both routes are local-peer gated with no admin escape hatch: a link a phone could redeem would carry a permanent identity past ADR 040, so an off-machine refusal is audited as the signal that would earn a cross-device design. The item carries no `frozenBy` because its design was frozen in a spec rather than an ADR — which is exactly why the roadmap-truth drift check could not watch it. The arc kept moving the same day it was marked shipped: the office overlay became a steerable member reel — everyone in the room, wheel/arrow/chevron navigation, lane owners first (#469); izzo\u2019s acceptance-run find (the spent sign-in nonce surviving the address bar on the success path) was fixed by stripping through the router\u2019s own history so nothing can re-stamp it (#470); and the scene\u2019s chrome — reel card, speech bubbles, nameplates — was recut from warm paper with measured AA contrast (#473). Remainder: the travel-animation increment stays held behind its cut-line.',
    refs: [adr(145, 'ADR 145'), adr(98, 'ADR 098')],
    dependsOn: ['human-role-reevaluation', 'insight-dashboard'],
  },
  {
    id: 'two-stage-close',
    title: 'Two-stage close — awaiting acceptance + counterpart outcome accept',
    shipped: { prs: [436, 437, 440, 442] },
    frozenBy: 169,
    category: 'human-loop',
    blurb:
      'Split "done" into the worker’s claim (`awaiting_acceptance` after merge) and a counterpart’s outcome acceptance (intent/principles/usable/feel — not a code review). Missing acceptor → self-close recorded unconfirmed, never a wedge. Vocabulary clarified in ADR 192; mechanics from ADR 169.',
    detail:
      '`resolve` (ADR 025) conflates two claims — the worker’s "technically complete" and the owner’s "this is what I wanted". The founder: agent work can be technically done but not what he wanted when he reviews it with his own eyes (the agile sprint-demo acceptance moment). So the worker asserts only a post-merge claim; a *different* seat (acceptor) co-signs before `done`, and a self-close marks unconfirmed. The acceptance request is an ordinary ask-stream item with a timeout, so a missing acceptor degrades to self-close-flagged-unconfirmed rather than wedging. Keeps every settled constraint of the resolve-as-state-gate brainstorm: musterd runs no verifiers, threads can’t wedge, verified-ness is derived from a counterpart act, never a stored second flag. Rides on human-ask-stream. SHIPPED 2026-07-28 as ADR 169, increments 1–4: the frozen design (#436), the stage-one state with verified-ness derived from the audit log rather than stored (#437), the submit verbs + unconfirmed-close nudge (#440), and the board surface (#442). ADR 192 (2026-07-31) renames agent-facing vocabulary off "review" (`awaiting_acceptance` / `lane_submit` / accept·reject) and puts an intent/principles/usable/feel checklist in the ask — audit action strings stay frozen. Increment 5 (an ephemeral cross-family acceptor) is parked with its blocker in §5: it is *earned* (3 of 3 ready edges degraded, zero asks routed) but unbuildable while no cross-family arm exists on the machine. So the close works and every self-close now reads unconfirmed; what is missing is a second family to route to. Its first live data also corrected the ADR — a zero catch rate cannot mean rubber-stamping where the candidate pool can be empty, making the no-candidate rate a first-class metric.',
    refs: [
      adr(145, 'ADR 145'),
      adr(25, 'ADR 025'),
      adr(192, 'ADR 192'),
  ...
  },
  {
    ...
    building:
      'nothing is in flight — the toolkit migration (ADR 272 §4, under its era name `profiles`), the only build the ADR authorizes, shipped in #921 (pure rename, built-in round-trip) and #922 (label and charter decouple from the `profile`). What remains is §5 — the four-level registry, Git reconciliation, role-addressed sends, and the ordered routing resolver — DEFERRED, not building: it reopens only when ADR 227’s restored measured trigger fires (the role-filtered discovery→directed-send join; 2026-08-19 measurement was zero `roster.role_query` rows, re-run 2026-09-16 found 2, and the join has still not fired as specified) or a multi-holder pick is repeatedly contested by hand. If role sends do proceed, increment 1 rides the shipped ADR 254 eligible-set rail rather than a new resolver.',
    wave: 8,
    title: 'Roles vs toolkits — responsibility without harness lock-in, routing behind its gate',
    plan: 'near-term',
    category: 'platform',
    blurb:
      'Keep roles as team-owned responsibilities and move harness setup into separate local workspace toolkits; role routing stays deferred behind measured demand.',
    detail:
      'ADR 272 (revised 2026-08-19 after the second ADR 227 measurement — zero role-discovery usage ever, so the pre-registered reopening trigger for role-addressed sends had vacuously never fired; the original registry/resolver scope was conceded as unevidenced): the seat is the only required entity; a role is an optional team-side responsibility (charter, narrow-only capability defaults, approved holders — ADR 227 unchanged); a workspace provisioning toolkit (ADR 272’s `profile`, renamed by ADR 296) is optional local setup that neither grants nor removes a role and never carries team authority. The one authorized build is migrating the ADR 026–030 provisioning layer to the `profile` vocabulary and boundary (now `toolkit`, ADR 296). The four-level registry, Git reconciliation, role-addressed sends, and ordered routing resolver are deferred behind ADR 227’s restored measured trigger or demonstrated multi-holder contention — and when role sends do proceed, the first increment resolves holders onto the shipped ADR 254 eligible-set rail (named seats, any one discharges) rather than a new resolver. Named-seat accountability and human-admin authority over durable assignments (ADR 145) survive as constraints on any future routing build. A 2026-09-16 re-run of the eval SQL found 2 `roster.role_query` rows (both 2026-08-21); the discovery→send join has still not fired as specified, so §5 stays deferred.',
  ...
  },
  {
    ...
    detail:
      'Captured 2026-07-13 (owner intent; full brainstorm/design session pending — docs/design/roles-and-stewardship.md is the seed). The prompting problem: any agent can restart/rebuild shared infrastructure or modify platform code while teammates are online and depending on it; the desired end state is that only designated platform agent(s) may, and everyone else routes infra requests + troubleshooting to them (request_help by role). Explicitly lenient while the team is still building musterd itself — warn-first, watcher-never-gatekeeper, hardening later. Concrete first target (captured 2026-07-20): the **platform / infra guardian** as an on-call, self-healing-prod agent — a local cheap-probe-wakes-a-session watcher that supervises the daemon and the ADR 152 auto-refresher, auto-remediates safe classes, and escalates the rest, with the per-incident autonomy tier set by an admin via `musterd team policy` (ADR 150). Runtime/recency/probe/autonomy-as-policy all captured in the seed doc; the daemon auto-refresh mechanism it supervises shipped as ADR 152. Builds on what exists rather than re-inventing: the ADR 069/070 capability substrate (roles/<name>.toml already carries defaults + charter, per-seat narrowing, in-band enforcement + audit), the ADR 026–030 provisioning toolkits (the per-harness rendering half, under their era name `templates`), the steward (ADR 112) as the first worked example of a role-agent (charter + autonomy knobs + guardrails; today an Action, wants to be a resident seat via ADR 131), and the no-orchestrator stance (a role is charter + capabilities on an ordinary seat, never a new protocol power). Roles are harness-independent by design — the same role assignable on Claude Code, Codex, Cursor, or musterd’s own future harness. Seed library also includes a Facilitator/brainstorm archetype (tldraw for diverge, harness visual companion for converge; portable intent, per-harness surfaces) — see the seed doc. Scope widened 2026-08-04 (AgentField survey, landscape.md §9): the design session must also answer **capability-tagged discovery and routing-by-role** — the seed doc’s own Q1 (role assignment; can a seat hold more than one?) and Q3 (role-addressed acts) as first-class requirements, not follow-ups. Today the roster carries the liveness half (presence, posture, quiescence ADR 219, wakeable ADR 131) but no aptitude half: ADR 069/070 capabilities are restrictions (may this seat send/observe/admin), never affordances (is this seat good at X); Member.role is one free-text string validated against nothing; team_members filters by exact name only; charter is prose nothing indexes. The design should yield a selector shape (“quiet + wakeable + can do X” — ADR 191’s wake-eligibility selector is the precedent, capability-blind today) and role-as-addressable-pool for team_send, so “ask the platform agent” stops requiring tribal knowledge of who that is. **Design frozen 2026-08-04 by ADR 227** (session: nick + izzo): a role stays charter + narrow-only capability defaults on an ordinary seat, now multi-role (`roles: string[]`, first = display label, roleless = generalist default) with a required one-line summary; increment 1 is discovery (`team_members` role filter + roles + summary + the liveness trio — the selector, with NO new wire recipient kind); increment 2 is the warn-only infra-touch gate (names current `platform` holders, `infra.touch.warned` audit event, never blocks; the warn → --force → refuse ramp documented but evidence-gated and admin-flipped); v1 library is four live roles — platform → izzo (stanley alternate), designer → miley (the standing frontend rule as a charter), steward (ADR 112 re-anchored), observer → wanderer (the ADR 144 inc-5 special case folded in) — with the rest of the wishlist staying unheld role definitions. Deliberately deferred on named reopening triggers: role-addressed sends (reopens when dogfood shows senders re-implementing the holder pick), the guardian on-call agent (its own later ADR), autonomy-tiers-as-team-policy, and free capability tags. Admin stays a human-only capability (ADR 145), never a role — the narrow-only clamp means no role file can mint one. **SHIPPED 2026-08-04, both increments in one day.** Increment 1 (#650): seat files take `roles = [...]` (single-role files stay byte-identical; `role` remains the always-emitted display label), role files gained a one-line `summary`, `mergeRoleDefaults` composes held roles restrictively (a grant survives silent roles; a restriction in any held role holds), `team_members` gained a role filter that leads with the role summary and appends the ADR 219 quiescence read beside the existing wakeable facet — the "quiet + wakeable + holds X" selector in one query — plus `musterd role assign`, migration v31, epoch 7. Roster truth warns on unknown names in the new array form only; legacy bare labels ("tester") stay unvalidated so the warn channel does not get flooded. Increment 2 (#654): the warn-only infra-touch gate — `GET /teams/:slug/infra-gate` where the DAEMON owns both halves (decides whether the caller holds `platform` AND writes the `infra.touch.warned` audit row), wired into `service install|restart|refresh` and `reset`; the CLI half is a silence contract (human shells, unbound folders, an unreachable daemon, an old daemon 404, a malformed body all collapse to nothing, 1.5s cap) so a warning can never stall or block the verb that fixes health. The v1 library went live on the revive roster the same day: platform → izzo, designer → miley (the standing frontend rule as a charter), steward (ADR 112 re-anchored, still the Action), observer → wanderer (the ADR 144 inc-5 special case folded in). STILL DEFERRED on their named triggers: role-addressed sends (reopens when telemetry shows the two-call query-then-send pattern repeatedly), the platform guardian on-call agent (its own ADR), autonomy-tiers-as-team-policy, free capability tags, and the hardening ramp warn → --force → refuse (gated on the measured warn→redirect rate).',
    refs: [
      adr(227, 'ADR 227'),
      adr(70, 'ADRs 069–070'),
      adr(112, 'ADR 112'),
